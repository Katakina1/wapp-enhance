...
public class EinvoiceQueryServiceImpl implements EinvoiceQueryService {
    ...
    @Override
    public PageUtils queryInvoiceMsg(String schemaLabel, EinvoiceQueryEntity queryEntity, Long userId) {
        //获取当前页面
        ...
        taxNos.add(Constants.DEFAULT_ELECTRON_INVOICE_GF_TAX_NO);

        queryEntity.setTaxNos(taxNos);

        List<ElectronInvoiceEntity> invoiceList = this.queryList(schemaLabel, queryEntity);

        int total = this.queryTotal(schemaLabel, queryEntity);

        return new PageUtils(invoiceList, total, queryEntity.getLimit(), page);
    }
    ...
    @Override
    public int queryTotal(String schemaLabel, EinvoiceQueryEntity queryEntity) {
