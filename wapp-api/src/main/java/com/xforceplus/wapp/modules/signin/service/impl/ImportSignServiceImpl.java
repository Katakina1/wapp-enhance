...
public class ImportSignServiceImpl implements ImportSignService {
     ...
     */
    @Override
    public Integer getInvoiceDouble(Map<String, String> ocrMap) {
        return signImportDao.getInvoiceDouble(null, ocrMap);
    }
    ...
    public String getInvoiceImage(Map<String, String> params) {
        ...
        try {
            if (!StringUtils.isEmpty(invoiceImage)) {
                ...
                imageHandler.download(invoiceImage, userAccount + Constant.FILE_TYPE_ZIP);
    ...
    private InvoiceScan buildInvoiceScan(ExportEntity exportEntity, ResponseInvoice responseInvoice, RecordInvoiceEntity entity) {
        ...
        invoiceScan.setBarCode(entity.getBarCode());
        invoiceScan.setScanMatchStatus(entity.getScanMatchStatus());
//        invoiceScan.setIsExistStamper(entity.getIsExistStamper());
...
//            flag = signImportDao.insertInvoiceImgforCustomer(exportEntity.getSchemaLabel(), imgMapList) > 0;
    ...
    private RecordInvoiceEntity buildRecordByMap(Map<String, String> map, String scanId) {
        ...
        if (StringUtils.isNotEmpty(map.get("totalAmount"))) {
            entity.setTotalAmount(new BigDecimal(map.get("totalAmount")));
        } else {
    ...
    private Date formatterDate(String date) {
        SimpleDateFormat sdf = new SimpleDateFormat(Constant.SHORT_DATE_FORMAT);
        try {
    ...
    private File zipImg(String imgPath) throws IOException {
        ...
        final String path = new SimpleDateFormat(Constant.SHORT_DATE_FORMAT).format(new Date()) + "_" + UUID.randomUUID().toString().replace("-", "") + ".zip";
...
//
//                //是否有红章
//                invoiceVo.setIsExistStamper(ocrMap.get("isExistStamper"));
            //文件类型
            ...
            try {
                if ("2".equals(invoiceVo.getBilltypeCode())) {
                    String bpmsAndCost = "";
                    String barCode = "";
                    if ("3".equals(isImage)) {
                        LOGGER.info("扫描识别到封面----------------------");
                        LOGGER.info("打印参数----------------------" + ocrMap);
                        if (StringUtils.isNotBlank(ocrMap.get("barCode"))) {
                            String[] orcBarcode = ocrMap.get("barCode").split("-");
                            barCode = orcBarcode[orcBarcode.length - 1];
                            bpmsAndCost = orcBarcode[orcBarcode.length - 2];
                            LOGGER.info("barCode1----------------------" + ocrMap.get("barCode"));
                            LOGGER.info("barCode2----------------------" + barCode);
                            LOGGER.info("bpmsAndCost----------------------" + bpmsAndCost);
//                                if(barCode.length()>10){
//                                    signImportDao.updateCostScanDate(barCode);
//                                }else{
//                                    signImportDao.updateEpsCostScanDate(barCode);
//                                }

                        }
                        //给所有当天扫描但是scantwodate未赋值的费用单据赋值
                        signImportDao.updateAllCostScanDate();
                    } else {
                        String[] orcArrayName = ocrMap.get("arrayName").split("-");
                        barCode = orcArrayName[orcArrayName.length - 1];
                        bpmsAndCost = orcArrayName[orcArrayName.length - 2];
                    }
                    if (StringUtils.isNotBlank(bpmsAndCost) && bpmsAndCost.equals("2")) {
                        barCode = signImportDao.getCostNoBySettlement(null, barCode);
                        signImportDao.updateCostScanDate(barCode);
                    } else {
                        if (barCode.length() < 10) {
                            barCode = signImportDao.getCostNoBySettlement(null, barCode);
                    ...
                    invoiceVo.setBarCode(barCode);
                }
            } catch (Exception e) {
                invoiceVo.setBarCode(null);
            }
            ...
            if (isImage.equals("1") && "10".equals(CommonUtil.getFplx(invoiceVo.getInvoiceCode()))) {
                invoiceVo.setFileType("2");
            }
            ...
            try {
                invoiceVo.setTotalAmount(new BigDecimal(ocrMap.get("totalAmount")));
            } catch (Exception e) {
    ...
    public Map<String, Object> getUpdateRecordInvoiceEntity(ExportEntity exportEntity, Map<String, Object> invoicess) {
        ...
        if (recordInvoiceEntity != null) {
            ...
            recordInvoiceEntity.setBarCode((String) invoicess.get("costNo"));
            String type = (String) invoicess.get("flowType");
            String nbr = (String) invoicess.get("invoiceNo");
            String code = (String) invoicess.get("invoiceCode");
            String xfTaxNo = (String) invoicess.get("xfTaxNo");
            String venderId = "";
            //商品获取匹配表的venderId,费用表获取费用发票表venderId,如果没有则通过销方税号获取
            if (StringUtils.isNotEmpty(type)) {
                if (type.equals("1")) {
                    venderId = signImportDao.getSpVenderId(code, nbr);
                    if (StringUtils.isEmpty(venderId)) {
                        venderId = signImportDao.getOrgVenderId(xfTaxNo);
                    }
                } else if (type.equals("2")) {
                    if (null != invoicess.get("venderid") && !invoicess.get("venderid").equals("null")) {
                        venderId = (String) invoicess.get("venderid");
                    }
                    if (StringUtils.isEmpty(venderId)) {
                        venderId = signImportDao.getCoseVenderId(code, nbr);
                        if (StringUtils.isEmpty(venderId)) {
                            venderId = signImportDao.getOrgVenderId(xfTaxNo);
                        }
                    }
            ...
            recordInvoiceEntity.setVenderid(venderId);
//            if(null!=invoicess.get("venderid")&&!invoicess.get("venderid").equals("null")){
...
//            recordInvoiceEntity.setIsExistStamper((String)invoicess.get("isExistStamper"));
//            try {
//            recordInvoiceEntity.setNoExistStamperNotes((String)invoicess.get("noExistStamperNotes"));
//            }catch (Exception e){
//                recordInvoiceEntity.setNoExistStamperNotes(null);
//            }
            try {
            ...
            try {
                ...
                recordInvoiceEntity.setTotalAmount(bigDecimal);
            } catch (Exception e) {
                try {
                    recordInvoiceEntity.setTotalAmount(new BigDecimal((String) invoicess.get("totalAmount")));
                } catch (Exception e2) {
                    e2.printStackTrace();
            ...
            try {
                ...
                e3.printStackTrace();
                recordInvoiceEntity.setCheckCode(null);
            }
...
//        final List<RecordInvoiceEntity> costInvoiceList = newArrayList();
        ...
        recordInvoiceEntityList.forEach((RecordInvoiceEntity recordInvoiceEntity) -> {
            ...
            //名
            recordInvoiceEntity.setInvoiceTypeName(InvoiceTypeEnum.invoiceTypeMap().get(invoiceType));
...
//                        }
                ...
                //如果是费用附件取供应商号
                if ("2".equals(recordInvoiceEntity.getFileType()) && "2".equals(recordInvoiceEntity.getBilltypeCode())) {
                    //t_dx_settlement
                    String venderId = signImportDao.getVenderIdByCostNo(null, recordInvoiceEntity.getBarCode());
                    recordInvoiceEntity.setVenderid(venderId);
                }
...
//        map.put("costInvoiceList", costInvoiceList);
    ...
    private Map<String, Object> tomeInvoiceHandle(ExportEntity exportEntity, List<RecordInvoiceEntity> recordInvoiceEntityList, String importType, Boolean modifyFlag) {
        ...
        for (RecordInvoiceEntity entity : recordInvoiceEntityList) {
            //取费用表的费用号
            try {
                String costNo = signImportDao.getCostNo(entity.getInvoiceCode(), entity.getInvoiceNo());
                LOGGER.info("发票:" + entity.getInvoiceNo() + "费用号:" + costNo);
                if (StringUtils.isNotEmpty(costNo)) {
                    entity.setBarCode(costNo);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            //取机构表的jvcode
            List<String> orgcode = signImportDao.getOrgCode(entity.getGfTaxNo(), entity.getBilltypeCode());
            if (orgcode != null && orgcode.size() > 0) {
                if (entity.getBilltypeCode().equals("1")) {
                    for (int j = 0; j < orgcode.size(); j++) {
                        String orgcodeString = orgcode.get(j);
                        if (orgcodeString.equals("UZ")) {
                            orgcode.remove(j);
                            break;
                        }
                        if (orgcodeString.equals("OZ")) {
                            orgcode.remove(j);
                            break;
                        }
                    }
            ...
            if("8".equals(entity.getBilltypeCode())){
                entity.setJvCode("");
                entity.setCompanyCode("");
                if (!modifyFlag ) {
                    entity.setVenderid("");
            ...
            if (StringUtils.isNotBlank(entity.getDyInvoiceNo()) && StringUtils.isNotBlank(entity.getInvoiceNo()) &&
                ...
                if (dataEntity != null || entity.getInvoiceType().equals("04")) {
                    if (dataEntity != null) {
                        if (!StringUtils.isEmpty(dataEntity.getMatchno())) {
                            LOGGER.info("matcNo为:" + dataEntity.getMatchno());
                            if (!dataEntity.getGfTaxNo().equals(entity.getGfTaxNo())) {
                                //不一致签收失败
                                //赋值 签收失败
                                entity.setQsStatus(SignInEnum.QS_FAIL.getValue());
                                //失败描述
                                entity.setNotes("签收失败，发票购方税号不一致");
                            ...
                            List<String> code = signImportDao.getOrgCode(dataEntity.getGfTaxNo(), "2");
                            boolean isJv = false;
                            if (StringUtils.isBlank(dataEntity.getJvcode())) {
                                dataEntity.setJvcode(entity.getJvCode());
                            }
                            for (String co : code) {
                                if (dataEntity.getJvcode().equals(co)) {
                                    isJv = true;
                                }
                            }
                            if (!isJv) {
                                ...
                                entity.setQsStatus(SignInEnum.QS_FAIL.getValue());
                                //失败描述
                                entity.setNotes("签收失败，购方税号与匹配JV不符");
                                //构建扫描实体 将解析出来的发票信息存到扫描表中 （签收状态（签收失败）签收方式（导入签收）签收时间（空））以便在签收处理菜单进行签收。
                                final InvoiceScan invoiceScan = buildInvoiceScan(exportEntity, dataEntity, entity, Boolean.FALSE);
                                //放入集合
                                invoiceScanErrorList.add(invoiceScan);
                    ...
                    if (!"7".equals(entity.getBilltypeCode())) {
                        ...
                        if ((dataEntity == null && "04".equals(entity.getInvoiceType())) || "2".equals(dataEntity.getSourceSystem()) || ("0".equals(dataEntity.getSourceSystem()) && "0".equals(dataEntity.getDetailYesorno()))) {
                            if (dataEntity == null && "04".equals(entity.getInvoiceType())) {
                                checkIndex = 1;
                            } else if (dataEntity != null && "04".equals(entity.getInvoiceType())) {
                                checkIndex = 2;
                            } else if (dataEntity != null && "01".equals(entity.getInvoiceType())) {
                                checkIndex = 3;
                            }
                            ...
                            if (responseInvoice != null && ReturnInfoEnum.CHECK_SUCCESS.getResultCode().equals(responseInvoice.getResultCode())) {
                                ...
                                if (checkIndex == 1) {
                                    //无敌张普票
                                    newGfTaxNo = responseInvoice.getBuyerTaxNo();
                                } else if (checkIndex == 2) {
                                    //有底账普票
                                    newGfTaxNo = responseInvoice.getBuyerTaxNo();
                                } else if (checkIndex == 3) {
                                    if (null != dataEntity.getGfTaxNo()) {
                                        if (dataEntity.getGfTaxNo().length() > responseInvoice.getBuyerTaxNo().length()) {
                                            newGfTaxNo = dataEntity.getGfTaxNo();
                                            dataEntity.setNewGfTaxno(dataEntity.getGfTaxNo());
                                        } else {
                                            newGfTaxNo = responseInvoice.getBuyerTaxNo();
                                            dataEntity.setNewGfTaxno(responseInvoice.getBuyerTaxNo());
                                        }
                                    } else {
                                        newGfTaxNo = responseInvoice.getBuyerTaxNo();
                                    }
                                ...
                                if (dataEntity == null) {
                                    ...
                                    inspectionProcess(exportEntity.getSchemaLabel(), responseInvoiceList, true, newGfTaxNo);
                                    //重新获取底账
                                    dataEntity = signImportDao.getRecordData(exportEntity.getSchemaLabel(), entity.getInvoiceCode() + entity.getInvoiceNo());
                                    try {
                                        LOGGER.info("JV为:" + dataEntity.getJvcode());
                                    } catch (Exception e) {
                                    ...
                                    dataEntity.setDetailYesorno("1");
                                    inspectionProcess(exportEntity.getSchemaLabel(), responseInvoiceList, false, newGfTaxNo);
                                    //重新获取底账
                                    dataEntity = signImportDao.getRecordData(exportEntity.getSchemaLabel(), entity.getInvoiceCode() + entity.getInvoiceNo());
                                    try {
                                        LOGGER.info("JV为:" + dataEntity.getJvcode());
                    ...
                    if (!modifyFlag) {
                        ...
                        entity.setTotalAmount(dataEntity.getTotalAmount());
                    } else {
                        //如果是修改若购方、销方税号跟底账一致则取底账的购方、销方名称
                        if (entity.getGfTaxNo() != null && entity.getGfTaxNo().equals(dataEntity.getGfTaxNo())) {
                            entity.setGfName(dataEntity.getGfName());
                        }
                        if (entity.getXfTaxNo() != null && entity.getXfTaxNo().equals(dataEntity.getXfTaxNo())) {
                            entity.setXfName(dataEntity.getXfName());
                        }
                    }
                    ...
                    //取机构表的jvcode
                    orgcode = signImportDao.getOrgCode(entity.getGfTaxNo(), entity.getBilltypeCode());
                    if (orgcode != null && orgcode.size() > 0) {
                        if (entity.getBilltypeCode().equals("1")) {
                            for (int j = 0; j < orgcode.size(); j++) {
                                String orgcodeString = orgcode.get(j);
                    ...
                    //商品的JV一定是WI
                    try {
                        if (entity.getGfTaxNo().equals("914403007109368585") & entity.getBilltypeCode().equals("1")) {
                            dataEntity.setJvcode("WI");
                            entity.setJvCode("WI");
                        }
                    } catch (Exception e) {
                        LOGGER.info("税号或业务类型为空");
                        e.printStackTrace();
                    }
                    //入库
                    if (isrukCJV) {
                    ...
                    if (consistentFlag) {
                        entity.setScanMatchStatus(dataEntity.getScanMatchStatus());
                        //对比一致
                        //判断业务底账类型是否为空
                        if (StringUtils.isBlank(dataEntity.getFlowType())&&!entity.getBilltypeCode().equals("8")) {
                            //如果底账表业务类型为null用扫描/修改的业务类型覆盖底账表的
                        ...
                        if (StringUtils.isBlank(entity.getBilltypeCode()) || !entity.getBilltypeCode().equals(dataEntity.getFlowType())) {
                            ...
                            entity.setQsStatus(SignInEnum.QS_FAIL.getValue());
                            //失败描述
                            entity.setNotes(entity.getBilltypeCode().equals("8")?"签收失败，不属于IP2P":"签收失败，所选类型与发票业务类型不一致");
                            //构建扫描实体 将解析出来的发票信息存到扫描表中 （签收状态（签收失败）签收方式（导入签收）签收时间（空））以便在签收处理菜单进行签收。
                            final InvoiceScan invoiceScan = buildInvoiceScan(exportEntity, dataEntity, entity, Boolean.FALSE);
                            //放入集合
                            invoiceScanErrorList.add(invoiceScan);
                        ...
                        if (!dataEntity.getFlowType().equals("1")) {
                            ...
                            if (getTaxAmountFlag) {
                                ...
                                if(entity.getBilltypeCode().equals("8")) {
                                    if (modifyFlag && (entity.getInvoiceType().equals("08") || entity.getInvoiceType().equals("10"))) {

                                    } else {
                                        //判断是否已收票
                                        int signInMarkQuery = signImportDao.signInMarkQuery(entity.getInvoiceCode(), entity.getInvoiceNo());
                            ...
                            if (StringUtils.isNotBlank(dataEntity.getTaxRate())) {
                                ...
                                try {
                                    BigDecimal abc = new BigDecimal(dzTaxTate);
                                } catch (Exception e) {
                                    dzTaxTate = "0";
                                }
                                BigDecimal sl = new BigDecimal(dzTaxTate).divide(new BigDecimal("100"));
                                BigDecimal chazhi = entity.getInvoiceAmount().multiply(sl).subtract(dataEntity.getTaxAmount()).setScale(2, BigDecimal.ROUND_HALF_UP);
                                //最大值0.05  最小值-0.05
                                ...
                                if (chazhi.compareTo(max) == 1 || chazhi.compareTo(min) == -1) {
                                    ...
                                    if (StringUtils.isNotBlank(dataEntity.getLslbz())) {
                                        ...
                                        if (dkmx) {
                                            ...
                                            dataEntity.setDeductibleTax(dkTaxAmount.toPlainString());
                                            dataEntity.setDeductibleTaxRate(shuiLv);
...
//                                                entity.setNotes("签收失败，该供应商号下商品与明细不匹配，无法计算抵扣税额");
                                                ...
                                                if (dataEntity.getInvoiceType().equals("04")) {
                                                    if (StringUtils.isBlank(dataEntity.getDeductibleTax())) {
                                                        //普票抵扣税率为null赋值0
                                                        dataEntity.setDeductibleTax("0");
                                                        dataEntity.setDeductibleTaxRate("0");
                                                        //保存0抵扣
                                                        signImportDao.saveDeductibleTax(exportEntity.getSchemaLabel(), dataEntity);

                                                    }
                                                    if (StringUtils.isNotBlank(dataEntity.getDkInvoiceAmount())) {
                                                        BigDecimal totalAmount = entity.getInvoiceAmount().add(entity.getTaxAmount());
                                                        if (new BigDecimal(dataEntity.getDkInvoiceAmount()).compareTo(totalAmount) == 0) {
                                                            //对比一致 则签收成功 赋值 签收成功
                                                            entity.setQsStatus(SignInEnum.QS_SUCCESS.getValue());
                                                            entity.setNotes("签收成功");
                                                            //将解析出来的发票信息存到扫描表中 （签收状态（签收成功）签收方式（导入签收）签收时间当前时间）
                                                            ...
                                                            entity.setQsStatus(SignInEnum.QS_FAIL.getValue());
                                                            //失败描述
                                                            entity.setNotes("签收失败，抵扣金额应等于发票合计金额");
                                                            //抵账表不存在 构建扫描实体 将解析出来的发票信息存到扫描表中 （签收状态（签收失败）签收方式（导入签收）签收时间（空））以便在签收处理菜单进行签收。
                                                            final InvoiceScan invoiceScan = buildInvoiceScan(exportEntity, dataEntity, entity, Boolean.FALSE);
                                                            //放入集合
                                                            invoiceScanErrorList.add(invoiceScan);
                                        ...
                                        //普票计算抵扣金额与发票金额的对比 若抵扣税率存在抵扣金额要小于发票金额否不存在则抵扣金额等于发票金额
                                        if (dataEntity.getInvoiceType().equals("04")) {
                                            Boolean deductibleTaxBoolean = StringUtils.isBlank(dataEntity.getDeductibleTax());
                                            if (deductibleTaxBoolean) {
                                                //普票抵扣税率为null赋值0
                                                dataEntity.setDeductibleTax("0");
                                                dataEntity.setDeductibleTaxRate("0");
                                                //保存0抵扣
    ...
    private Boolean checkListRecordInvoiceEntity(RecordInvoiceEntity recordInvoiceEntity) {
        ...
        if (recordInvoiceEntity.getInvoiceDate() != null) {
            SimpleDateFormat simpleDateFormat2 = new SimpleDateFormat(Constant.DEFAULT_SHORT_DATE_FORMAT);
            dateFlag = CommonUtil.isValidDate(simpleDateFormat2.format(recordInvoiceEntity.getInvoiceDate()), Constant.DEFAULT_SHORT_DATE_FORMAT, "[0-9]{4}-[0-9]{2}-[0-9]{2}");
            if (!dateFlag) {
                recordInvoiceEntity.setQsStatus(SignInEnum.QS_FAIL.getValue());
        ...
        } else if (Arrays.asList(Constant.TOME_INVOICE_TYPE).contains(invoiceType)) {
            //普票
...
//        invoiceScan.setIsExistStamper(entity.getIsExistStamper());
    ...
    private Boolean checkInfoConsistent(RecordInvoiceEntity entity, RecordInvoiceDataEntity dataEntity, List<String> finalTaxNoList) {
        ...
        if("8".equals(entity.getBilltypeCode())) {
            if (!"0".equals(dataEntity.getInvoiceStatus())) {
                entity.setNotes("发票状态异常");
        ...
        if ((dataEntity.getTotalAmount() == null && entity.getTotalAmount() != null) ||
                dataEntity.getTotalAmount() != null && entity.getTotalAmount() == null) {
            entity.setNotes("价税合计不一致");
            consistentFlag = Boolean.FALSE;
        }
        ...
        if (!finalTaxNoList.contains(gf)) {
            if("8".equals(entity.getBilltypeCode())) {
                //ariba还需要查看一下Ariba税号维护表
                Integer aribaTaxCount = signImportDao.getAribaTaxCount(gf);
                if (aribaTaxCount==null||aribaTaxCount == 0) {
...
//			}
    ...
    @Transactional
    @Override
    public Boolean inspectionProcess(String schemaLabel, List<ResponseInvoice> responseInvoiceList, boolean cyYoN, String dzGfTaxno) {
        ...
        responseInvoiceList.forEach(responseInvoice -> {
            ...
            invoiceCollectionInfo.setGfTaxNo(dzGfTaxno == null ? responseInvoice.getBuyerTaxNo() : dzGfTaxno);
            if (inspectionDetailInfoList.size() > 0) {
                //保存明细表
                //构建明细表数据
                //保存明细数据
                try {
                    ...
                    for (int j = 0; j < groudList.size(); j++) {
                        noDetailedInvoiceDao.insertNoDetailedInvoice(schemaLabel, groudList.get(j));
                    }
    ...
    private InvoiceCollectionInfo buildInvoiceCollectionInfo(ResponseInvoice responseInvoice, String buyerTaxNo) {
        ...
        SimpleDateFormat simpleDateFormatyyyyMMdd = new SimpleDateFormat(Constant.SHORT_DATE_FORMAT);
        //开票日期
        ...
        //有明细
        invoiceCollectionInfo.setDetailYesorno(Constant.DETAIL_YES_OR_NO);
