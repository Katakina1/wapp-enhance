...
var currentQueryParam = {
    ...
    xfName: null,
    invoiceNo: null,
    bbindingDate1: new Date().getFullYear()+"-"+format2(new Date().getMonth()+1)+"-01",
    bbindingDate2: new Date().getFullYear()+"-"+format2(new Date().getMonth()+1)+"-"+format2(new Date().getDate()),
    invoiceStatus: "-1",
    invoiceType: "-1",
    qsStatus: "-1",
    rzhYesorno: "-1",
...
var vm = new Vue({
    ...
    data:{
        ...
        totalAmount: 0,
        totalTax: 0,
        bbindingDateOptions1: {},
        bbindingDateOptions2: {},
        qsDateOptions1: {},
        qsDateOptions2: {},
        rzhDateOptions1: {},
        rzhDateOptions2: {},
        ...
        updateWin:false,
        rules:{
            // bbindingNo:[{ type: 'number',message: '请输入正确的装订册号',trigger: 'blur'}],
            invoiceNo:[{
    ...
    mounted:function(){
        this.bbindingDateOptions1 = {
            disabledDate: function(time){
                var currentTime = new Date(vm.formatDate(vm.form.bbindingDate2));
                return time.getTime() >= currentTime;
            }

        };
        this.bbindingDateOptions2 = {
            disabledDate: function(time){
                var currentTime = new Date(vm.formatDate(vm.form.bbindingDate1));
    ...
    methods: {
        changeFun: function(selection){
            ...
            if(selection.length==0){
                $("#export_btn2").attr("disabled","disabled").addClass("is-disabled");
        ...
        saveRoleData:function() {
            ...
            var url = "pack/inputPackNumber/uplist";
            var no = vm.upform.packingNo;
            // alert(no);
            $.ajax({
                type: "POST",
                url: baseURL + url,
                contentType: "application/json",
                data: JSON.stringify({bbindingNos:roleIds,packingNo:no}),
                success: function (r) {
                    if (r.code === 0) {
                        // vm.reloadRole();
                        vm.upform.packingNo = null;
                        vm.updateWin = false;
                        alert('录入成功');
        ...
        batchImport: function (event) {
            if (!this.isNeedFileExtension) {
                ...
                var flag = false
                var hh;
                var url = baseURL + "pack/inputPackNumber/enterPackageNumber";
                this.$http.post(url, formData, config).then(function (response) {
                    ...
                    if (response.data.success) {
                        alert("共计导入" + (response.data.reason + response.data.errorCount) + "条，成功" + response.data.reason + "条，未填装箱号"+response.data.errorCount+"条"
                        );
                        this.selectFileFlag = '';
                        this.file = '';
                        $("#upload_form")[0].reset();
        ...
        query: function (formName) {
            ...
            var qsStartDate = new Date(vm.form.bbindingDate1);
            var qsEndDate = new Date(vm.form.bbindingDate2);
            ...
            if ( qsEndDate.getTime()+1000*60*60*24 > qsStartDate.getTime()) {
                ...
                checkKPDate=false;
            }else if(qsEndDate.getTime() < new Date(vm.form.bbindingDate1)){
            ...
            this.$refs[formName].validate(function (valid) {
                if (valid) {
                    currentQueryParam = {
                        ...
                        'bbindingDate1': vm.form.bbindingDate1,
                        'bbindingDate2': vm.form.bbindingDate2,
        ...
        exportExcel: function(){
            ...
            // document.getElementById("ifile").src = encodeURI(baseURL + 'pack/inputPackNumber/exportExcel'
            //     +'?&epsNo='+(currentQueryParam.epsNo==null?'':currentQueryParam.epsNo)
            //     +'&invoiceSerialNo='+(currentQueryParam.invoiceSerialNo==null?'':currentQueryParam.invoiceSerialNo)
            //     +'&bbindingDate1='+currentQueryParam.bbindingDate1
            //     +'&bbindingDate2='+currentQueryParam.bbindingDate2
            //     +'&bbindingNo='+(currentQueryParam.bbindingNo==null?'':currentQueryParam.bbindingNo)
            //
            // );
            ...
            $.ajax({
                ...
                type:"POST",
                data:{'serviceType':21,'condition':JSON.stringify(params)},
                success:function (r) {
                    flag = true;
                    if(r.code==0){
                        alert(r.data);
        ...
        detailVehicleFormCancel: function () {
            ...
            vm.detailForm.bbindingDate = null;
        ...
        findAll: function (currentPage) {
            ...
            this.$http.post(baseURL + 'pack/inputPackNumber/list',
                currentQueryParam,
                {
                    'headers': {
                        "token": token
        ...
        detail1:function(row){
            var  value = row.id;
            var bindingNo =row.bbindingNo;
            $('.col-xs-3 tr td:contains("异常")').parent('tr').css('display','none');
            var params = {
                'bbindingNo': bindingNo,
                'id': value,
                'page': 1,
                'limit': 50
            };
            vm.detailFormVisible = true;
            this.$http.post(baseURL + 'pack/inputPackNumber/bindingnolist',
                params,
                {
                    'headers': {
                        "token": token
...
function getSelectedRows() {
    ...
    for (var i = 0; i < selection.length; i++) {
        ids.push(selection[i].bbindingNo);
