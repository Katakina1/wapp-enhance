       ...
       /* queryDetail:function(){
        ...
        changeFunPO:function(row) {
            ...
            for(var i=0;i<row.length;i++){
                if(row[0].venderid ==row[i].venderid){
                    details.push(row[i].matchno);
                    vm.venderid = row[i].venderid;
                }else {
                    alert("请选择相同的供应商号的数据！");
                    details=[];
                }
            ...
            if(vm.selection.length>0){
                //增加样式 export_btn_pdf
                $("#export_btn_pdfGF").removeAttr("disabled").removeClass("is-disabled");
            }else {
                $("#export_btn_pdfGF").attr("disabled","disabled").addClass("is-disabled");
            }
        },
        exportPoPDF:function(){
            var uri = baseURL + 'export/invoicePoRepertoire/invoicePoListExportGF' +'?matchnoList='+JSON.stringify(vm.selection)+'&venderid='+vm.venderid ;
            $.ajax({
                type: "POST",
