...
function verificationPoCodeValue(t) {
    var reg = /[^\d]/g;
    vm.form.poCode = t.value.replace(reg, '');
};
function verificationclaimNoValue(t) {
    var reg = /[^\d]/g;
    vm.form.claimNo = t.value.replace(reg, '');
};
    ...
    methods: {
        ...
        findAll: function (currentPage) {
            ...
            this.$http.post(baseURL  + 'modules/posuopei/matchQuery/list',
                ...
                if(vm.tableData.length>0){
                    $("#export_btn_sss").removeAttr("disabled").removeClass("is-disabled");
                }
            });
        ...
        changeFunPO:function (row) {
            ...
            for (var i = 0; i < row.length; i++) {
                details.push(row[i].matchno);
                venderNmae.push(row[i].venderName);
            }
            vm.venderNmae = venderNmae;
            vm.selection = details;
            if (vm.selection.length > 0) {
                if (vm.selection.length > 1) {
                    alert("请不要选择多个选项！");
                    $("#export_btn_pdf").attr("disabled", "disabled").addClass("is-disabled")
                    return;
                }
                //增加样式 export_btn_pdf
                $("#export_btn_pdf").removeAttr("disabled").removeClass("is-disabled");
            } else {
                $("#export_btn_pdf").attr("disabled", "disabled").addClass("is-disabled");
            }
        },
        exportPoPDF: function () {
            var checkuUri = baseURL + 'export/invoicePoRepertoire/checkCover' + '?matchnoList=' + JSON.stringify(vm.selection) + '&venderName=' + JSON.stringify(vm.venderNmae);
            $.ajax({
                type: "POST",
                url: checkuUri,
                ...
                success: function (r) {
                    if (r.code == 0) {
                        var uri = baseURL + "export/invoicePoRepertoire/invoiceChaXunExport?matchnoList=" + JSON.stringify(vm.selection) + '&venderName=' + JSON.stringify(vm.venderNmae);
                        $.ajax({
                            type: "POST",
        ...
        exportExcel: function(){
            ...
            $.ajax({
                ...
                type:"POST",
                data:{'serviceType':29,'condition':JSON.stringify(params)},
                success:function (r) {
                    flag = true;
                    if(r.code==0){
                        alert(r.data);
            ...
            /*document.getElementById("ifile").src = encodeURI(baseURL + 'export/invoicePoRepertoire/queryExport'
                +'?venderid='+ vm.form.venderid
                +'&poCode='+ vm.form.poCode
                +'&claimNo='+ vm.form.claimNo
                +'&invoiceNo='+ vm.form.invoiceNo
            ...
            $("#export_btn_sss").attr("disabled","true").addClass("is-disabled");
        },
        ...
        exportData:function(){
            document.getElementById("ifile").src = baseURL + sysUrl.matchImportExportWo;
        },
        ...
        uploadFile: function (event) {
            if (!this.isNeedFileExtension) {
                ...
                formData.append('venderid','0')
                // formData.append('venderName',this.queryData1.username)
                formData.append('orgtype','5')
        ...
        exportInvExcel:function () {
            document.getElementById("ifile").src = encodeURI(baseURL + 'export/invoicePoRepertoire/exportDetail'
                +'?matchno='+ vm.importMatchNo
            );
        },
        exportPoExcel:function () {
            document.getElementById("ifile").src = encodeURI(baseURL + 'export/invoicePoRepertoire/exportDetailPo'
                +'?matchno='+ vm.importMatchNo
            );
        },
        exportClaimExcel:function () {
            document.getElementById("ifile").src = encodeURI(baseURL + 'export/invoicePoRepertoire/exportDetailClaim'
                +'?matchno='+ vm.importMatchNo
            );
        },
        write: function () {
