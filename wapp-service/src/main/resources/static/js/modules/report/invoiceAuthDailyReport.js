...
$(function () {
    ...
    $("#jqGrid").jqGrid({
        url: baseURL + 'report/invoiceAuthDailyReport/list',
        datatype: "json",
        postData: {
        ...
        colModel: [
            { label: '认证日期', name: 'rzhDate', sortable:false, align: 'center'},
        ...
        loadComplete: function(xhr){
            ...
            vm.form.rzhBelongDate = xhr.rzhBelongDate;
            currentQueryParam.gfName = xhr.gfName;
            currentQueryParam.rzhBelongDate = xhr.rzhBelongDate;

            //不为空说明是第一次进页面,后台将此值带入
            if(xhr.defaultRzhBelongDate != null) {
                defaultRzhBelongDate = xhr.defaultRzhBelongDate;
            }

            createAmountGraph(xhr.reportList);
            createCountGraph(xhr.reportList);

            if(xhr.reportList == null || xhr.reportList.length == 0){
...
var vm = new Vue({
    ...
    methods: {
                ...
                for(var i=0;i<r.optionList.length;i++){
                    ...
                    if(i==0){
                        ...
                        $.get(baseURL + 'report/invoiceAuthDailyReport/currentTaxPeriod',
                            {taxNo: gf.value},
                            function(res){
                                vm.form.rzhBelongDate = ''+res;
                                currentQueryParam.rzhBelongDate = ''+res;
                                defaultRzhBelongDate = ''+res;
                            });
        ...
        exportExcel: function(){
            document.getElementById("ifile").src = baseURL + 'export/invoiceAuthDailyReportExport'
