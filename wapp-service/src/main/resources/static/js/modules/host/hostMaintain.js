...
var vm = new Vue({
    ...
    methods: {
        ...
        findAll: function (currentPage) {
            ...
            vm.listLoading = true;
            var flag = false;
            this.$http.post(baseURL  + 'modules/job/matchQuery/list',
                params,
                {
                    'headers': {
                        "token": token
        ...
        exportExcel: function(){
           // $("#export_btn").attr("disabled","true").addClass("is-disabled");
            ...
            $.ajax({
                ...
                type:"POST",
                data:{'serviceType':56,'condition':JSON.stringify(params)},
                success:function (r) {
                    flag = true;
                    if(r.code==0){
                        alert(r.data);
        ...
        resultDetail:function (row) {
            ...
            $.ajax({
                        type: "POST",
                        url: baseURL + "modules/job/matchQuery/write",
                        contentType: "application/json",
...
function getSelectedRows() {
    var selection = vm.multipleSelection2;
    ...
    for (var i = 0; i < selection.length; i++) {
        if(selection[i].hoststatus=='A0010'||selection[i].hoststatus=='1'||selection[i].hoststatus=='9'||selection[i].hoststatus=='10'||selection[i].matchingType=='6'||selection[i].hoststatus=='0'||selection[i].hoststatus=='8'||selection[i].hoststatus=='B0001'||selection[i].hoststatus=='B0002'||selection[i].hoststatus=='B0003'||selection[i].hoststatus=='B0004'){
          return null;
        }else{
