    ...
    methods: {
        ...
        release:function () {
           var isExist=0;
            ...
            if(isExist<5){
                alert('请先确保保存PC、MD、PC&MD、PFR、COUPON五个模板，然后再发布!');
                return;
            }
            ...
            confirm('确定要发布？', function () {
                ...
                $.ajax({
                    type: "POST",
                    url: baseURL + "couponAnnouncement/release",
                    contentType: "application/json",
                    success: function (r) {
                        loading.close();
                        if (r.code == 0) {
        ...
        findAll: function (currentPage) {
            ...
            var flag = false;
            var hh;
            this.$http.post(baseURL + "couponAnnouncement/list",
                params_, {
                    'headers': {
                        "token": token
                    }
        ...
        exportFailureData() {
            document.getElementById("ifile").src = baseURL + 'export/couponFailure';
        },
        exportSuccessfulData() {
            ...
            document.getElementById("ifile").src = baseURL + 'export/coupon'+ '?' + $.param(params_);
        },
        exportData() {
        ...
        debtTemplate:function(){
            document.getElementById("ifile").src = baseURL +"export/couponTemplate";
        },
        ...
        deleteBatch: function () {
            confirm('确定要清空列表中所有的数据？', function () {
                $.ajax({
                    type: "POST",
                    url: baseURL + "announcementCoupon/delete",
                    contentType: "application/json",
                    success: function (r) {
                        if (r.code == 0) {
                            vm.findAll(1);
...
$(function () {
    ...
    new AjaxUpload('#import', {
        action: baseURL + 'export/couponAnnouncementUpload' ,
        name: 'file',
        autoSubmit: true,
