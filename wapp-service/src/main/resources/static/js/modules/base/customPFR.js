...
var vm = new Vue({
    ...
    methods: {
        ...
        findAll: function (currentPage) {
            ...
            var flag = false;
            var hh;
            this.$http.post(baseURL + "customPFR/list",
                params_, {
                    'headers': {
                        "token": token
                    }
        ...
        exportFailureData() {
            document.getElementById("ifile").src = baseURL + 'export/PFRFailureList';
        },
        exportSuccessfulData() {
            ...
            document.getElementById("ifile").src = baseURL + 'export/PFRSuccessful'+ '?' + $.param(params_);
        },
        exportData() {
        ...
        debtTemplate:function(){
            document.getElementById("ifile").src = baseURL +"export/PFRTemplate";
        },
        ...
        deleteBatch: function () {
            confirm('确定要清空所有的数据？', function () {
                $.ajax({
                    type: "POST",
                    url: baseURL + "announcementPFR/delete",
                    contentType: "application/json",
                    success: function (r) {
                        if (r.code == 0) {
                            vm.findAll(1);
...
$(function () {
    ...
    new AjaxUpload('#import', {
        action: baseURL + 'export/customPFR' ,
        name: 'file',
        autoSubmit: true,
        data:{type:"protocol"},
