     ...
     dateFormat: function(row, column, cellValue, index){
        ...
        revokeCount: function() {
            ...
            for(var i=0; i<this.multipleSelection.length; i++) {
                ...
                var cxtjStatus = this.multipleSelection[i].cxtjStatus
                if (sfdqskssq!='1' || tjStatus!='2' || (qsStatus!='0' && qsStatus!='3') || (cxtjStatus!='0' && cxtjStatus !='3')){
            ...
            confirm('确定要撤销统计选中的记录？', function(){
                $.ajax({
                    type: "GET",
                    url: baseURL + "certification/deductStatistics/revokeCount",
        ...
        revokeConfirm: function() {
            ...
            for(var i=0; i<this.multipleSelection.length; i++) {
                ...
                var tjStatus = this.multipleSelection[i].tjStatus;
                var qsStatus = this.multipleSelection[i].qsStatus;
                var cxqsStatus = this.multipleSelection[i].cxqsStatus;
                if (sfdqskssq!='1' ||  qsStatus!='2' || (cxqsStatus!='0' && cxqsStatus !='3')){
                    alert("所选数据中存在不符合条件的数据，请重新选择");
                    return;
                }
                gfshs.push(this.multipleSelection[i].taxno);
            ...
            confirm('确定要撤销确认选中的记录？', function(){
                $.ajax({
                    type: "GET",
                    url: baseURL + "certification/deductStatistics/revokeConfirm",
                    contentType: "application/json",
                    data: {gfshs:gfshArr,skssqs:skssqArr},
                    success: function(r){
                        console.log(r);
