...
var vm = new Vue({
    ...
    methods: {
        ...
        findAll: function (currentPage) {
            ...
            // validateOnlyNumber(vm.form.invoiceNo, 0, "sixgod", "发票号码只能为8位以内数字", 8);
            ...
            vm.listLoading = true;
            var flag = false;
            this.$http.post(baseURL + 'modules/fixed/matchQuery/list',
                params,
                {
                    'headers': {
                        "token": token
         ...
         * 格式化审核时间
         */
        formatCheckhDate: function (row, column) {
            if (row.checkDate != null) {
                return dateFormat(row.checkDate);
            } else {
                return '—— ——';
            }
        },
        ...
        exportInvoice:function () {
            var url = baseURL+'export/fixed/invoiceDetailExport?';
            var data = { //一个json数据
                // page: currentPage,
                // limit: this.pageSize,
                // venderid: vm.form.venderid,
            ...
            console.log(result);
            document.getElementById("ifile").src = result;
        },
        matchDetail: function (row) {
            var params = {matchId: row.id};
            this.$http.post(baseURL + 'modules/fixed/matchQuery/detail',
                params,
                {
                    'headers': {
                        "token": token
        ...
        MatchCancel: function (row) {
            ...
            $.post(baseURL + "modules/fixed/matchQuery/cancel", data, function (r) {
                alert(r.msg);
        ...
        scanWinShow:function (row) {
            ...
            this.$http.post(baseURL + "modules/fixed/matchQuery/fileInfo",
                params, {
                    'headers': {
                        "token": token
                    }
        ...
        showImg:function(row){
            ...
            $.ajax({
                ...
                success: function (r) {
                    ...
                    document.getElementById("viewInvoicesImg").src = baseURL + 'modules/fixed/matchQuery/getImageForAll?id=' + id + "&token=" + token;
                    setTimeout(function(){ vm.loadingPic = false;},2000)
                },
                error: function () {
        ...
        downloadFile : function(row){
            ...
            $.ajax({
                ...
                contentType: "application/json",
                success: function (r) {
                    document.getElementById("downloadFileId").src = encodeURI(baseURL + 'modules/fixed/matchQuery/downloadFile?id='+row.id + "&token=" + token);
                },
                error: function () {
        ...
        detailFormCancelA: function () {
            ...
            document.getElementById("viewInvoicesImg").src ="";
        },
