...
var vm = new Vue({
    ...
    methods: {
        ...
        print:function() {
            ...
            vm.upform.refundReason ='';
            vm.upform.refundRemark ='';
                    var url = baseURL + "export/printQueryPayCostList"+'?matchnoList='+JSON.stringify(vm.multipleSelection )
        ...
        exportExcel: function(){
            ...
            // document.getElementById("ifile").src = encodeURI(baseURL + 'export/refundCostExport'
            //     +'?&venderId='+(currentQueryParam.venderId==null?'':currentQueryParam.venderId)
            //     +'&rebateNo='+(currentQueryParam.rebateNo==null?'':currentQueryParam.rebateNo)
            ...
            var params ={
                ...
                'rebateNo':(currentQueryParam.rebateNo==null?'':currentQueryParam.rebateNo),
                'epsNo':(currentQueryParam.epsNo==null?'':currentQueryParam.epsNo),
                'shopNo':(currentQueryParam.shopNo==null?'':currentQueryParam.shopNo),
                'rebateDate1':currentQueryParam.rebateDate1,
                'rebateDate2':currentQueryParam.rebateDate2
            };
            $.ajax({
                ...
                type:"POST",
                data:{'serviceType':55,'condition':JSON.stringify(params)},
                success:function (r) {
                    flag = true;
                    if(r.code==0){
                        alert(r.data);
        ...
        findAll: function (currentPage) {
            ...
            this.$http.post(baseURL + 'scanRefund/printRefundInformationCost/list',
                currentQueryParam,
                {
                    'headers': {
                        "token": token
