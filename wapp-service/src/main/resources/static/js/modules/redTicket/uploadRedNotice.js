...
var vm = new Vue({
    ...
    data:{
        ...
        detailDialogVehicleFormVisibles:false,
        token: token,
    ...
    methods: {
        ...
        detailFormCancels: function () {
            vm.detailDialogVehicleFormVisibles = false;
            vm.detailForms.tkDate = null;
            vm.detailForms.xfName = null;
            vm.detailForms.gfName = null;
            vm.detailForms.xfTaxno = null;
            vm.detailForms.gfTaxno = null;
            vm.detailForms.amount = null;
            vm.detailForms.taxRate = null;
            vm.detailForms.taxAmount = null;
            vm.detailForms.redNoticeNumber = null;
        },
        details: function (row) {
            vm.detailForms.tkDate = row.tkDate;
            vm.detailForms.xfName = row.xfName;
            vm.detailForms.gfName = row.gfName;
            vm.detailForms.xfTaxno = row.xfTaxno;
            vm.detailForms.gfTaxno = row.gfTaxno;
            vm.detailForms.amount = row.amount;
            vm.detailForms.taxRate = row.taxRate;
            vm.detailForms.taxAmount = row.taxAmount;
            vm.detailForms.redNoticeNumber = row.redNoticeNumber;
            vm.detailDialogVehicleFormVisibles = true;
        },
        ...
        uploadFiles: function (event) {
            if (!this.isNeedFileExtension) {
                ...
                this.tempTableData = this.tableDatas;
                this.tableDatas = [];
                ...
                this.$http.post(url, formData, config).then(function (response) {
                        ...
                        if (response.data.success) {
                            if (this.tempTableData.length + response.data.reason.length + response.data.errorCount + response.data.errorCount1 + response.data.errorCount2 > 500) {
                                this.tableDatas = this.tempTableData;
                            ...
                            var str4 = "";
                            if (response.data.errorEntityList4.length > 0) {

                                for (var i = 0; i < response.data.errorEntityList4.length; i++) {
                                    str4 = str4 + response.data.errorEntityList4[i].redTicketDataSerialNumber + "、";
                                }
                            ...
                            alert("共计导入" + (response.data.reason.length + response.data.errorCount + response.data.errorCount1 + response.data.errorCount2 + response.data.errorEntityList3.length+ response.data.errorEntityList4.length) + "条，成功" + response.data.reason.length + "条,库里已存在红字通知单:" + response.data.errorCount1 + "条,对应的序列号：" + str + ",excel表格重复" +
                                response.data.errorCount2 + "条" + str2 + ",数据错误" + response.data.errorEntityList3.length + "条，对应序列号" + str3+ ",未审核或审核不同意" + response.data.errorEntityList4.length + "条，对应序列号" + str4);
                            vm.querys();
                        } else {
        ...
        currentChanges: function (currentPage) {
            if (vm.total == 0) {
                return;
            }
            this.findAlls(currentPage);
        },
        findAlls: function (currentPage) {
            currentQueryParams.page = currentPage;
            currentQueryParams.limit = vm.pageSize;
            this.listLoadings = true;
            var flag = false;
        ...
        handleSizeChanges: function (val) {
            this.pageSize = val;
            if (!isInitials) {
                this.findAlls(1);
            }
        ...
        queryOpenRedTicketType: function () {
            $.get(baseURL + 'modules/openRedInvoiceQuery/queryOpenRedTicketType', function (r) {
                ...
                for (var i = 0; i < r.optionList.length+1; i++) {
                    var gf = {};
        ...
        detailFormCancelDelect: function () {
        ...
        findAll: function (currentPage) {
            ...
            this.$http.post(baseURL + 'modules/upload/list/uploadNoticeQueryPaged',
                currentQueryParam,
                {
                    'headers': {
                        "token": token
        ...
        exportData:function(){
            document.getElementById("ifile").src = baseURL + "export/redTicket/noticeImportExport";
        },
        //查看上传资料
       ...
       /* submitUpload() {
        ...
        check: function (row) {
            vm.aId = row.id;
            vm.redTicketDataSerialNumber = row.redTicketDataSerialNumber;
            vm.whetherOpenRedticket = row.whetherOpenRedticket;
            vm.businessType = row.businessType;
            vm.detailDialogRedNoticePicture = true;
            vm.redNoticeAssociation = row.redNoticeAssociation;
            var para = {
                'redNoticeAssociation': vm.redNoticeAssociation
            };
            vm. findNoticeFileList(para);
        ...
        findNoticeFileList:function(para){


            this.$http.post(baseURL + 'modules/openRedTicket/queryOpenRedNoticeList',
                para,
                {
                    'headers': {
                        "token": token
                ...
                vm.tableNoticeFileList = xhr.fileList;
        ...
        //删除文件
        deleteRedNoticeFile:function(row){
            var para = {
                'id': row.id,
                'businessType': vm.businessType,
                'redTicketDataSerialNumber': vm.redTicketDataSerialNumber,
            ...
            this.$http.post(baseURL + 'modules/uploadRedNotice/deleteRedNoticeFile',
                para,
                {
                    'headers': {
                        "token": token
        ...
        uploadFile: function (event) {
            if ($("#showFileName").html() == "未选择文件") {
                ...
                alert("上传的文件不能大于2M！");
            } else if (vm.tableNoticeFileList.length >0) {
                alert("请先删除关联的pdf文件，再上传！");
            }else {
                //event.preventDefault();
                var formData = new FormData();
                formData.append('file', this.file);
