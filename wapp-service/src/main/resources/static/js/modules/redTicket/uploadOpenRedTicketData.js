...
var vm = new Vue({
    ...
    methods: {
        ...
        findAll: function (currentPage) {
            ...
            this.$http.post(baseURL + 'modules/openRedTicket/list/queryUploadPaged',
                currentQueryParam,
                {
                    'headers': {
                        "token": token
        ...
        //删除文件
        deleteRedDataFile:function(row){
            var para = {
                'id': row.id,
                'filePath': row.filePath
            };
            this.$http.post(baseURL + 'modules/openRedTicket/deleteRedDataFile',
                para,
                {
                    'headers': {
                        "token": token
        ...
        onChangeFile: function (event) {
            ...
            if (photoExt != '' && !( photoExt == '.pdf' || photoExt == '.png' || photoExt == '.gif' || photoExt == '.jpg'|| photoExt1 == '.jpeg')) {
                alert("请上传pdf/png/gif/jpeg/jpg文件!");
                $("#file").val("");
