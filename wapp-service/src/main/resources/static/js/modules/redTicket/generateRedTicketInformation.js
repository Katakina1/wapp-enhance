            ...
            handler: function (val, oldValue) {
                ...
                if (!(reg.test(val) || val == null)) {
                    Vue.nextTick(function () {
                        _this.form.invoiceCode = oldValue;
                    })
                }
            },
            deep: true
    ...
    methods: {
        ...
        query: function (formName) {
            ...
            if ( new Date(nowtime)>new Date(vm.form.invoiceDate3)) {
                $("#requireMsg4 .el-form-item__content div.el-date-editor:first-child").append('<div class="checkMsg">已超出可申请期限</div>');
                checkKPDate=false;
            ...
            for(var t=0;t<vm.returnGoods.length;t++){
                if(vm.returnGoods[t].returnGoodsDate<nowtime){
            ...
            if(Math.abs(vm.sumReturnAmount)<=0){
                ...
                if(Math.abs(vm.sumRedRushAmount)<=0){
                    ...
                    if(Math.abs(vm.sumReturnAmount)<vm.sumRedRushAmount){
                        alert("所选红冲的总金额大于所选退货的成本金额，请重新选择");
                        return false;
                    }
                    else {
                        currentQueryParam = {
                            ...
                            'returnGoods': vm.returnGoods,
                            'sumRedRushAmount':vm.sumRedRushAmount,
                            "businessType":'1'
                        };
        ...
        updateInvoiceDetails: function () {
            ...
            if(vm.invoiceDetails1.length>0) {
                ...
                for (var i = vm.invoiceDetails1.length - 1; i >= 0; i--) {
                    for (var k = 0; k < vm.invoiceDetails1.length; k++) {
                        if (vm.invoiceDetails1[i].goodsName == vm.invoiceDetails1[k].goodsName &&
                            ...
                            if (parseInt(vm.invoiceDetails1[i].unitPrice) > parseInt(vm.invoiceDetails1[k].unitPrice)) {
                    ...
                    // vm.invoiceDetails1[i].taxAmount=(vm.invoiceDetails1[i].redRushAmount*(vm.invoiceDetails1[i].taxRate/100)).toFixed(2);
                }
                for (var a = 0; a < vm.invoiceDetails1.length; a++) {
                    redRushDetailsSumAmount = (redRushDetailsSumAmount * 100 / 100) + (vm.invoiceDetails1[a].redRushAmount * 100 / 100);
                }
                vm.sumRedRushAmount = redRushDetailsSumAmount.toFixed(2);
                ...
                for (var z = 0; z < tableData5.length - 1; z++) {
                    for (var j = tableData5.length - 1; j > z; j--) {
                        if (tableData5[z].goodsName == tableData5[j].goodsName &&
                            ...
                            tableData5[z].detailAmount = parseInt(tableData5[z].detailAmount) + parseInt(tableData5[j].detailAmount);
                            ...
                            tableData5[z].taxAmount = (tableData5[z].detailAmount * (tableData5[z].taxRate / 100)).toFixed(2);
        ...
        gfOrgCode:function(val){
            if(parseInt(vm.returnNum)>0||parseInt(vm.invoiceDetailNum)>0){
                location.reload();
            }
            if(val=='-1'){
                vm.gf.orgcode="";
        ...
        findAll1: function (currentPage3) {
            ...
            var flag = false;
            this.$http.post(baseURL + 'modules/invoice/list/query',
                currentQueryParam,
                {
                    'headers': {
                        "token": token
        ...
        findRAll: function (currentPage) {
            var params = {
                page: currentPage,
                limit: this.pageSize,
                returnGoodsCode:currentQueryParam.Code,
                orgname:vm.gf.orgcode,
                invoiceDate3: vm.form.invoiceDate3,
                invoiceDate4: vm.form.invoiceDate4,
            };
            this.listLoading = true;
            var flag = false;
            this.$http.post(baseURL + 'redTicket/GenerateRedTicketInformation/returnquery',
                params,
                {
                    'headers': {
                        "token": token
                ...
                /*for (var i=0;i<xhr.page.list.length;i++) {
                    this.tableData[i].businessType = vm.form.businessType;
                }*/
                this.listLoading = false;
        ...
        detail:function(row){
            ...
            if(buType == 1){
                ...
                this.$http.post(baseURL + 'modules/businessData/returngoodsdet/query',
                    params,
                    {
                        'headers': {
                            "token": token
        ...
        detail2:function(row){
            for (var k=0;k<vm.invoiceDetails1.length;k++){
                if(vm.invoiceDetails1[k].goodsName==row.goodsName&&
                    ...
                    vm.sumRedRushAmount=((vm.sumRedRushAmount*100/100)-(vm.invoiceDetails1[k].redRushAmount*100/100)).toFixed(2);
