...
var vm = new Vue({
    ...
    methods: {
        ...
        findAll: function (currentPage) {
            ...
            this.$http.post(baseURL + 'cost/query/lists',
                params,
                {
                    'headers': {
                        "token": token
        ...
        deleteData: function (row) {
            vm.openConfirm(vm, "\n" + "确定要退票选中的记录？", function () {
                ...
                $.ajax({
                    type: "POST",
                    url: baseURL + "cost/query/deleteRevoices",
                    contentType: "application/json",
                    data: JSON.stringify(params),
                    success: function (r) {
                        if (r.code == 0) {
        ...
        confirmDateCosts: function(row){
            vm.openConfirm(vm, "\n" + "确认选中福利发票的记录？", function () {
                var params = {
                    "costNo":row.costNo
                };
                $.ajax({
                    type: "POST",
                    url: baseURL + "scanQueryCost/confirmDateCosts",
                    contentType: "application/json",
                    data: JSON.stringify(params),
                    success: function (r) {
                        if (r.code == 0) {
