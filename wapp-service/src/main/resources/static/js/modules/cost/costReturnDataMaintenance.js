...
//复选框
        ...
        refundyesno:function (row) {
            ...
            confirm('确定要删除吗？', function () {
                $.ajax({
                    type: "POST",
                    url: baseURL + "modules/cost/questionnaire/refundyesnos",
        ...
        findAll: function (currentPage) {
            ...
            var flag = false;
            this.$http.post(baseURL + 'modules/cost/costReturnDataMaintenance/list',
                params,
                {
                    'headers': {
                        "token": token
         ...
         * */
        exportData:function(){
            document.getElementById("ifile").src = baseURL + "export/cost/questionnaireExport";
        },
        ...
        uploadFile: function (event) {
            if (!this.isNeedFileExtension) {
                ...
                var flag = false;
                var hh;
                var url = baseURL + "modules/cost/invoiceImport";
...
function getSelectedRows() {
    ...
    for (var i = 0; i < selection.length; i++) {
        ids.push(selection[i].applicantId)
