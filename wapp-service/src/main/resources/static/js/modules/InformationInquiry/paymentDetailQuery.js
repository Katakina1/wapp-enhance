...
var currentQueryParam = {
    ...
    referTo: "",
    jvcode: "",
    paymentDate1: new Date().getFullYear() + "-" + format2(new Date().getMonth() + 1) + "-01",
    paymentDate2: new Date().getFullYear() + "-" + format2(new Date().getMonth() + 1) + "-" + format2(new Date().getDate()),
    page: 1,
    limit: 1
};
...
var vm = new Vue({
        ...
        rules: {
            venderid: [{
                validator: function (rule, value, callback) {
                    ...
                    if (value != null && value !== "") {
                        if (!regex.test(value)) {
                            callback(new Error('必须为不超过6位的数值'))
                        } else {
                            callback();
                        }
                    } else{
            ...
            referTo:[{
                validator: function (rule, value, callback) {
                    ...
                    if (value != null && value !== "") {
                        if (!regex.test(value)) {
                            callback(new Error('必须为不超过12位的数值'))
                        } else {
                            callback();
                        }
                    } else{
            ...
            paymentDate1: [
                {type: 'string', required: true, message: '付款日期范围不能为空', trigger: 'change'}
            ],
            paymentDate2: [
                {type: 'string', required: true, message: '付款日期范围不能为空', trigger: 'change'}
            ]
    ...
    mounted: function () {
        this.queryDetails();
        this.paymentDateOptions1 = {
            disabledDate: function (time) {
                var currentTime = new Date(vm.formatDate(vm.form.paymentDate2));
                return time.getTime() >= currentTime;
            }

        };
        this.paymentDateOptions2 = {
            disabledDate: function (time) {
                var currentTime = new Date(vm.formatDate(vm.form.paymentDate1));
    ...
    methods: {
        ...
        venderIdNum: function () {
            vm.form.venderId = vm.form.venderId.replace(/[^\.\d]/g, '');
            vm.form.venderId = vm.form.venderId.replace('.', '');
        },
        ...
        query: function (formName) {
            ...
            this.$refs[formName].validate(function (valid) {
                if (valid) {
                    currentQueryParam = {
                        'referTo': vm.form.referTo,
                        'venderid': vm.form.venderid,
                        'jvcode': vm.form.jvcode,
                        'paymentDate1': vm.form.paymentDate1,
                        'paymentDate2': vm.form.paymentDate2,
                        'documentType':vm.form.documentType
                    };
                    vm.findAll(1);
                } else {
                    return false;
        ...
        paymentDate1Change: function (val) {
            vm.form.invoiceDate1 = val;
        },
        paymentDate2Change: function (val) {
        ...
        handlePost:function (content) {
            ...
            var url = baseURL + 'modules/InformationInquiry/paymentlist/upload';
        ...
        findAll: function () {
            var qsStartDate = new Date(this.form.paymentDate1);
            qsStartDate.setMonth(qsStartDate.getMonth() + 12);

            var qsEndDate = new Date(this.form.paymentDate2);
            if ( (qsEndDate.getTime() + 1000*60*60*24) > qsStartDate.getTime()) {
                ...
                return;
            } else if(qsEndDate.getTime()<new Date(this.form.paymentDate1)){
            ...
            var params = {
                ...
                referTo: currentQueryParam.referTo,
                jvcode: currentQueryParam.jvcode,
                paymentDate1: currentQueryParam.paymentDate1,
                paymentDate2: currentQueryParam.paymentDate2,
                documentType:currentQueryParam.documentType
            };
            this.listLoading = true;
            var flag = false;
            this.$http.post(baseURL + 'modules/InformationInquiry/paymentlist/query',
                params,
                {
                    'headers': {
                        "token": token
        ...
        dataFormat: function (row, column, cellValue, index) {
            if (cellValue == null) {
                return '—— ——';
            }
            return cellValue;
        ...
        exportExcelGF: function () {
            $("#export_btn").attr("disabled","true").addClass("is-disabled");
            // document.getElementById("ifile").src = baseURL + 'export/InformationInquiryPaymentExportGF'
            //     + '?venderid=' + this.form.venderid
            //     + '&referTo=' + this.form.referTo
            //     + '&jvcode=' + this.form.jvcode
            //     + '&paymentDate1=' + this.form.paymentDate1
            //     + '&paymentDate2=' + this.form.paymentDate2
            ...
            $.ajax({
                ...
                type:"POST",
                data:{'serviceType':25,'condition':JSON.stringify(params)},
                success:function (r) {
                    flag = true;
                    if(r.code==0){
                        alert(r.data);
    ...
    exportExcel: function () {
        ...
        $.ajax({
            url:baseURL + 'excel/apply',
            type:"POST",
            data:{'serviceType':12,'condition':JSON.stringify(currentQueryParam)},
            success:function (r) {
                flag = true;
                if(r.code==0){
                    alert(r.data);
