...
var vm = new Vue({
    ...
    methods: {
        ...
        invoiceDecimal1:function(row) {

            return vm.numberFormat(null,null, row.matchCover);
        } ,
        ...
        query: function () {
            ...
            var startDate = new Date(vm.form.matchDateStart);
            var endDate = new Date(vm.form.matchDateEnd);
            ...
            if ( endDate.getTime()+1000*60*60*24 > startDate.getTime()) {
                ...
                checkDate=false;
            }else if(endDate.getTime() < new Date(vm.form.matchDateStart)){
        ...
        findAll: function (currentPage) {
            ...
            vm.listLoading = true;
            var flag = false;
            this.$http.post(baseURL  + 'modules/InformationInquiry/matchQuery/list',
                params,
                {
                    'headers': {
                        "token": token
        ...
        changeFunPO:function(row) {
            ...
            if(vm.selection.length>0){
                //增加样式 export_btn_pdf
                $("#export_btn_excel").removeAttr("disabled").removeClass("is-disabled");
            }else {
                $("#export_btn_excel").attr("disabled","disabled").addClass("is-disabled");
            }
        },
        exportExcel:function(){
            var uri = baseURL + 'export/invoicePoExcel/invoicePoListExcel' +'?matchnoList='+JSON.stringify(vm.selection);
