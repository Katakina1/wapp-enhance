...
var vm=new Vue({
    ...
    data: {
        ...
        formInline: {
            invoiceNo: '',
            firstDate: getCurrentMonthDateTime(),
            lastDate: getCurrentDateTime1(),
            gfName: '',
            invoiceStatus : ''
        },
        pickerOptions0: [],
    ...
    methods: {
        ...
        findAll: function (currentPage) {
            ...
            var invoiceStartDate1 = new Date(vm.formInline.firstDate);
            var invoiceEndDate1 = new Date(vm.formInline.lastDate);

            invoiceStartDate1.setMonth(invoiceStartDate1.getMonth() + 12);

            if ( (invoiceEndDate1.getTime() + 1000*60*60*24) > invoiceStartDate1.getTime()) {
                $("#requireMsg4 .el-form-item__content div.el-date-editor:first-child").append('<div class="checkMsg">日期选择跨度不可超过1年</div>');
                checkKPDate=false;
            }else if(invoiceEndDate1.getTime() < new Date(vm.formInline.firstDate)){
            ...
            this.inportline.gfName=this.formInline.gfName;
            this.inportline.firstDate=this.formInline.firstDate;
            this.inportline.lastDate=this.formInline.lastDate;
            this.inportline.invoiceNo=this.formInline.invoiceNo;
            this.inportline.invoiceStatus=this.formInline.invoiceStatus;
            ...
            var params_ = {
                ...
                firstDate: this.formInline.firstDate == '' ? '' : this.formatDate(this.formInline.firstDate, true),
                lastDate: this.formInline.lastDate  == ''?'':this.formatDate(this.formInline.lastDate,true),
                invoiceNo: this.formInline.invoiceNo,
                invoiceStatus: this.formInline.invoiceStatus,

            };
            this.$http.post(baseURL + 'monit/abnormalinvoice/searchlist',
                 params_, {
                    'headers': {
                        "token": token
                    }
        ...
        formatDate: function (strTime) {
            ...
            if (row.statusUpdateDate != null) {
                return this.formatDate(row.statusUpdateDate);
            } else {
                return '';
            }
        ...
        formatInvoiceDate: function (row) {
            ...
            document.getElementById("ifile").src = baseURL + 'monit/toexcel'
                +'?gxTaxNo='+(this.inportline.gfName==null?'':this.inportline.gfName)
                +'&invoiceNo='+(this.inportline.invoiceNo==null?'':this.inportline.invoiceNo)
                +'&invoiceStatus='+(this.inportline.invoiceStatus==null?'':this.inportline.invoiceStatus)
                +'&firstDate='+(this.inportline.firstDate ==''?'':this.formatDate(this.inportline.firstDate,true))
                +'&lastDate='+(this.inportline.lastDate ==''?'':this.formatDate(this.inportline.lastDate,true))
                +'&token='+token;
        },
