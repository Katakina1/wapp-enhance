...
//        }
        ...
        r.put("invoiceEntity", invoiceEntity);
        r.put("outList", outList);
        r.put("detailAmountTotal",invoiceEntity.getInvoiceAmount());
        r.put("taxAmountTotal",invoiceEntity.getTaxAmount());
        return  r;
    }
    ...
    public R getInvoinceDetailList(@RequestBody String value){
        final String schemaLabel = getCurrentUserSchemaLabel();
        R.error("未找到明细信息");
        Long id=Long.parseLong(value);
        InvoiceEntity invoiceEntity=new InvoiceEntity();
        ...
        try{
            invoiceEntity=detailService.getInvoiceDetailList1(schemaLabel,id);
        }catch(Exception e){
            return R.error(1,"未查找到明细信息");
        }
        ...
        List<DetailEntity> detailEntityList=detailService.getInvoiceDetailList(schemaLabel, id);

        DecimalFormat df = new DecimalFormat("0.00");//格式化
        for (DetailEntity de:detailEntityList){
         Double cny = Double.parseDouble(de.getUnitPrice());//转换成Double
         de.setUnitPrice(df.format(cny));
        }
        r.put("detailEntityList",detailEntityList);
        r.put("invoiceEntity", invoiceEntity);
        r.put("outList", outList);
     ...
     */
    @SysLog("明细")
    @RequestMapping("/invoiceCheckDetail")
    public R getInvoinceCheckDetail(@RequestBody String value){
        final String schemaLabel = getCurrentUserSchemaLabel();
        R.error("未找到明细信息");
        InvoiceEntity invoiceEntity=new InvoiceEntity();
        if(StringUtils.isEmpty(value) || "null".equals(value)) {
        ...
        Long id=Long.parseLong(value);
        try{
            invoiceEntity=invoiceService.getCheckDetailInfo(schemaLabel,id);
        }catch(Exception e){
            return R.error(1,"未查找到明细信息");
        }
        ...
        if (flag.equals(vehicleInvoice)){
            DetailVehicleEntity detailVehicleEntity=new DetailVehicleEntity();
            try {
                detailVehicleEntity=detailService.getCheckVehicleDetail(schemaLabel, id);
            }catch (Exception e){
                LOGGER.error(e.toString());
            }
        ...
        List<DetailEntity> detailEntityList=detailService.getCheckInvoiceDetail(schemaLabel, id);
