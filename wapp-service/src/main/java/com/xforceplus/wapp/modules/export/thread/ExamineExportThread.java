 ...
 ******************************************************************************
 */
public class ExamineExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "examineAndUploadRedNotice";
	/**
	 * 文件名后缀
	 */
	...
	public ExamineExportThread(IExcelExportService exportLogService,
                               ExportLogEntity exportLog, ExamineAndUploadRedNoticeService examineAndUploadRedNoticeService,
	...
	public void run() {
			...
			try {
				...
				try {
					...
					Integer totalRowCount =examineAndUploadRedNoticeService.getRedTicketMatchListCount(map);
					...
					sheet.setClazz(ExamineExcelEntity.class);
					// 写数据 这个i的最大值直接拷贝就行了 不要改
					Integer a=1;
					List<ExamineExcelEntity> list2=new ArrayList<>();
					for (int i = 0; i < writeCount; i++) {
						...
						for (RedTicketMatch entity : list) {
							...
							if(tempList.size()>0){
								...
								ExamineExcelEntity en=examineAndUploadRedNoticeService.toEntity(redTicketMatchDetail,invoiceEntity,tax_sortcode,a++);
								list2.add(en);
							}

						}
						//List<ExamineExcelEntity> list2=examineAndUploadRedNoticeService.toExamineExcelEntity(mergeInvoiceDetailList,map);
