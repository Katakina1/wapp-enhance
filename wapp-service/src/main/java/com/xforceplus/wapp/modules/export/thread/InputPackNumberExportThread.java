 ...
 ******************************************************************************
 */
public class InputPackNumberExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "InputPackNumber";
	/**
	 * 文件名后缀
	 */
	...
	public InputPackNumberExportThread(IExcelExportService exportLogService,
                                       ExportLogEntity exportLog, InputPackNumberService inputPackNumberService,
                                       FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.inputPackNumberService = inputPackNumberService;
	...
	public void run() {
			...
			try {
				...
				try {
					...
					Integer totalRowCount =inputPackNumberService.queryTotalResult(userName,map);
					...
					sheet.setClazz(InputPackNumberExcelEntity.class);
					...
					for (int i = 0; i < writeCount; i++) {
						...
						List<InputPackNumberEntity> InvoiceEntityList = inputPackNumberService.queryList(userName,query1);
						List<InputPackNumberExcelEntity> list2=inputPackNumberService.transformExcle(InvoiceEntityList);
