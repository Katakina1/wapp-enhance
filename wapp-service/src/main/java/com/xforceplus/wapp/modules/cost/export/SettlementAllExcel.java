 ...
 * 订单信息导出
 */
public final class SettlementAllExcel extends AbstractExportExcel {
    ...
    public SettlementAllExcel(Map<String,Object> map, String excelTempPath, String excelName) {
        this.map = map;
    ...
    protected void buildExcel(XSSFWorkbook workBook) {
        ...
        for (SettlementEntity entity : list) {
            ...
            //费用金额
            setSheetValue(sheet, beginLine, 5, formatAmount(entity.getSettlementAmount().toString()), style);
            //EPS_NO
            setSheetValue(sheet, beginLine, 6, entity.getEpsNo(), style);
            //申请日期
            setSheetValue(sheet, beginLine, 7, formatDate(entity.getCreateDate()), style);
            //扫描匹配状态
            setSheetValue(sheet, beginLine, 8, getisDel(entity.getScanStatus()), style);
            //扫描日期
            setSheetValue(sheet, beginLine, 9, formatDate(entity.getScanDate()), style);
            //驳回理由
            setSheetValue(sheet, beginLine, 10, entity.getRejectReason(), style);
            beginLine++;
        }
    }
