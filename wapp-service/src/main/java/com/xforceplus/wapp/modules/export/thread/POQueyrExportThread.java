 ...
 ******************************************************************************
 */
public class POQueyrExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "订单查询导出";
	/**
	 * 文件名后缀
	 */
	...
	public POQueyrExportThread(IExcelExportService exportLogService,
                               ExportLogEntity exportLog, PoService poService,
	...
	public void run() {
		...
		try {
			...
			try {
				...
				Integer totalRowCount = poService.queryTotalResult( map);
				...
				for (int i = 0; i < writeCount; i++) {
					...
					List<PoEntity> InvoiceEntityList = poService.queryList( query1);
					List<PoExcelEntity> list2 = poService.transformExcle(InvoiceEntityList);
