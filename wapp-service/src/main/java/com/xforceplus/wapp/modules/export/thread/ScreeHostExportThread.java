...
import com.xforceplus.wapp.modules.posuopei.entity.ScreenExcelEntity;
 ...
 ******************************************************************************
 */
public class ScreeHostExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "ScreenHost";
	/**
	 * 文件名后缀
	 */
	...
	public ScreeHostExportThread(IExcelExportService exportLogService,
                                 ExportLogEntity exportLog, HostService hostService,
	...
	public void run() {
			...
			try {
				...
				try {
					...
					PagedQueryResult<MatchEntity> pagedQueryResult=hostService.getMatchEntityLists(map);
					Integer totalRowCount =pagedQueryResult.getTotalCount();
					...
					sheet.setClazz(ScreenExcelEntity.class);
					// 写数据 这个i的最大值直接拷贝就行了 不要改

					List<MatchEntity> claimEntityList = pagedQueryResult.getResults();
					//for (int i = 0; i < writeCount; i++) {
						List<ScreenExcelEntity> list2=hostService.transformExcle(claimEntityList);
