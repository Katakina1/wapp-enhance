...
public class CostApplicationServiceImpl implements CostApplicationService {
    ...
    public String submitAll(SettlementEntity settlement)  {
        //费用号
        String costNo = settlement.getCostNo();
        //合同
        String businessType = settlement.getBusinessType();
        try {
            settlement.setRemark(URLDecoder.decode(settlement.getRemark()));
            ...
            for (RecordInvoiceEntity invoice : settlement.getInvoiceList()) {
                ...
                if ("1".equals(invoice.getInvoiceKind()) || "2".equals(invoice.getInvoiceKind())) {
                    if (invoice.getIsExist()) {
                        //底账已有发票,更新费用号和费用发票标识
                        costApplicationDao.updateSettlementInvoice(costNo, invoice.getCoverAmount(), invoiceCode, invoiceNo, invoice.getJvcode(),invoice.getVenderid());
                    } else {
                        ...
                        if (entity != null) {
                            if("1".equals(invoice.getInvoiceKind())){
                ...
                for (RateEntity rate : invoice.getRateTableData()) {
                    ...
                    for (CostEntity cost : rate.getCostTableData()) {
                        ...
                        costDeptId = cost.getCostDeptId();
                        //匹配过来的,计算冲销金额等,并更新
                        if ("1".equals(settlement.getWalmartStatus())) {
                            costMatchDao.updateCostAmount(cost);
                            LOGGER.info("----------------冲销金额:"+cost.getCostAmount());
                            costMatchDao.updateSettlementAmount(cost);
                        }
                    }

                    costApplicationDao.getCostDeptId(costDeptId, invoiceCode, invoiceNo);
                }
            }
        ...
        if("0".equals(settlement.getWalmartStatus())){
            ...
            settlement.setBusinessType(businessType);
    ...
    public Map<String, Object> parseExcel(MultipartFile multipartFile) {
        ...
        try {
            ...
            //存税额
            List<String> rateTaxRateList = newArrayList();
            //存发票信息
            ...
            for(List<String> entity : list){
                ...
                String rateTaxRate =entity.get(9);
                ...
                if(!rateUuidList.contains(rateUuid) || !rateTaxRateList.contains(rateTaxRate)){
                    rateUuidList.add(rateUuid);
                    rateTaxRateList.add(rateTaxRate);
               ...
               /* else{
                    if(rateTaxRateList.contains(rateTaxRate)){
                        for(int i=0;i<rateTaxRateList.size();i++){
                            if(rateTaxRateList.get(i).equals(rateTaxRate)){
                                if(!rateUuidList.get(i).equals(rateUuid)){
                                    rateUuidList.add(rateUuid);
                                    rateTaxRateList.add(rateTaxRate);
                                    RateEntity rate = new RateEntity();
                                    rate.setInvoiceAmount(new BigDecimal(entity.get(8)));
            ...
            for(int a=0;a<rateListAll.size();a++){
                ...
                String rateTaxRate =rateTaxRateList.get(a);
                //费用列表
                List<CostEntity> costList = newArrayList();
                for(List<String> entity : list){
                    if(rateUuid.equals(entity.get(0)+entity.get(7)) && rateTaxRate.equals(entity.get(9))){
    ...
    public String getLoginname(Long loginname){
        return costApplicationDao.getLoginname(loginname);
    }
}
