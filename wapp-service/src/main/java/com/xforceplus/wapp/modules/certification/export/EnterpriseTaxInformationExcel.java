...
public final class EnterpriseTaxInformationExcel extends AbstractExportExcel{
    ...
    protected void buildExcel(XSSFWorkbook workBook) {
        ...
        for (EnterpriseTaxInformationEntity entity : list) {
            ...
            //当前税号
            setSheetValue(sheet, beginLine, 1, entity.getTaxNo(), style);
            //旧税号
            setSheetValue(sheet, beginLine, 2, entity.getOldTaxNo(), style);
            //企业名称
            setSheetValue(sheet, beginLine, 3, entity.getTaxName(), style);
            //当前税款所属期
            setSheetValue(sheet, beginLine, 4, formatStrDate(entity.getCurrentTaxPeriod()), style);
            //勾选开始日期
            setSheetValue(sheet, beginLine, 5, formatDate(entity.getSelectStartDate()),style);
            //勾选结束日期
            setSheetValue(sheet, beginLine, 6, formatDate(entity.getSelectEndDate()), style);
            //所属期结束日期
            setSheetValue(sheet, beginLine, 7, formatDate(entity.getOperationEndDate()), style);
            //信用等级
            setSheetValue(sheet, beginLine, 8, entity.getCreditRating(),style);
            //申报周期
            setSheetValue(sheet, beginLine, 9, formatDeclarePeriod(entity.getDeclarePeriod()), style);
            //jvccode
           /* setSheetValue(sheet, beginLine, 10, entity.getJvcode(),style);
            //companyCode
            setSheetValue(sheet, beginLine, 11, entity.getCompanyCode(), style);*/
            beginLine++;
        }
    }
