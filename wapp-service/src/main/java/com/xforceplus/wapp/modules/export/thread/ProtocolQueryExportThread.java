 ...
 ******************************************************************************
 */
public class ProtocolQueryExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "协议查询导出";
	/**
	 * 文件名后缀
	 */
	...
	public ProtocolQueryExportThread(IExcelExportService exportLogService,
                                     ExportLogEntity exportLog, ProtocolService protocolService,
	...
	public void run() {
		...
		try {
			...
			try {
				...
				Integer totalRowCount = protocolService.queryCountExport( map);
				...
				sheet.setClazz(ProtocolExcelEntity.class);
				...
				for (int i = 0; i < writeCount; i++) {
					...
					List<ProtocolEntity> InvoiceEntityList = protocolService.queryListExport( query1);
					List<ProtocolExcelEntity> list2 = protocolService.transformExcle(InvoiceEntityList);
