 ...
 ******************************************************************************
 */
public class RedTicketExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "RedTiketList";
	/**
	 * 文件名后缀
	 */
	...
	public RedTicketExportThread(IExcelExportService exportLogService,
                                 ExportLogEntity exportLog, InputRedTicketInformationService inputRedTicketInformationService,
                                 FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.inputRedTicketInformationService = inputRedTicketInformationService;
	...
	public void run() {
			...
			try {
				...
				try {
					...
					Integer totalRowCount =inputRedTicketInformationService.queryTotalResult(null,map).getTotalCount();
					...
					sheet.setClazz(UploadScarleQueryExcelEntity.class);
					...
					for (int i = 0; i < writeCount; i++) {
						...
						List<UploadScarletLetterEntity> list = inputRedTicketInformationService.queryList(null,query1);
						List<UploadScarleQueryExcelEntity> list2=inputRedTicketInformationService.toExcel(list);
