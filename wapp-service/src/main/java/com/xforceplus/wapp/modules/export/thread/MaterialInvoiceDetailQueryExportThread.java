 ...
 ******************************************************************************
 */
public class MaterialInvoiceDetailQueryExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "MaterialInvoiceQuery";
	/**
	 * 文件名后缀
	 */
	...
	public MaterialInvoiceDetailQueryExportThread(IExcelExportService exportLogService,
                                                  ExportLogEntity exportLog, MaterialInvoiceSubmitDetailService materialInvoiceSubmitDetailService,
                                                  FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.materialInvoiceSubmitDetailService = materialInvoiceSubmitDetailService;
	...
	public void run() {
			...
			try {
				...
				try {
					...
					Integer totalRowCount =materialInvoiceSubmitDetailService.queryMaterial(map).getTotalCount();
					...
					sheet.setClazz(MaterialInvoiceQueryExcelEntity.class);
					...
					for (int i = 0; i < writeCount; i++) {
						...
						List<ComprehensiveInvoiceQueryEntity> list = materialInvoiceSubmitDetailService.queryMaterial(query1).getResults();
						List<MaterialInvoiceQueryExcelEntity> list2=materialInvoiceSubmitDetailService.toExcel(list,map);
