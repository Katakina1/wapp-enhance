 ...
 * @date 10/20/2018
 */
public class AddPoImport extends AbstractImportExcel {

    private MultipartFile file;

    public AddPoImport(MultipartFile file) {
        this.file = file;
    }
    ...
    private AddClaimEntity createImportCertificationEntity(Row row, int index) {
        ...
        final String sysAmount = getCellData(row, 2);

        final String vendorAmount = getCellData(row, 3);

        final String num = getCellData(row, 4);

        final Boolean flag = this.checkInvoiceMessage(poCode,commodityNo, sysAmount, vendorAmount, num);
        if(flag){
            ...
            addClaimEntity.setDifference(new BigDecimal(vendorAmount).setScale(2).subtract(new BigDecimal(sysAmount).setScale(2)).multiply(new BigDecimal(num)).toString());
        }else{
    ...
    private Boolean checkInvoiceMessage(String poCode,String commodityNo, String sysAmount, String vendorAmount, String num) {
        Boolean flag=true;
        if(StringUtils.isEmpty(poCode)|StringUtils.isEmpty(sysAmount)|StringUtils.isEmpty(vendorAmount)|StringUtils.isEmpty(num)){
            flag=false;
        }
