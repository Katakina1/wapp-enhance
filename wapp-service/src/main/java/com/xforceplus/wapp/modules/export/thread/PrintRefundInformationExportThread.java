 ...
 ******************************************************************************
 */
public class PrintRefundInformationExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "printRefundInformationList";
	/**
	 * 文件名后缀
	 */
	...
	public PrintRefundInformationExportThread(IExcelExportService exportLogService,
                                              ExportLogEntity exportLog, PrintRefundInformationService printRefundInformationService,
                                              FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.printRefundInformationService = printRefundInformationService;
	...
	public void run() {
			...
			try {
				...
				try {
					...
					ReportStatisticsEntity reportStatisticsEntity =printRefundInformationService.queryTotalResult(userName,map);

					Integer totalRowCount =reportStatisticsEntity.getTotalCount();
					...
					sheet.setClazz(DytdfmExcelEntity.class);
					...
					for (int i = 0; i < writeCount; i++) {
						...
						List<EnterPackageNumberEntity> InvoiceEntityList = printRefundInformationService.queryList(userName,query1);
						List<DytdfmExcelEntity> list2=printRefundInformationService.transformExcle(InvoiceEntityList);
