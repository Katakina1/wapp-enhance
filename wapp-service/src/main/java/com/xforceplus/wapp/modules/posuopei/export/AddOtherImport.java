 ...
 * @date 10/20/2018
 */
public class AddOtherImport extends AbstractImportExcel {

    private MultipartFile file;

    public AddOtherImport(MultipartFile file) {
        this.file = file;
    }
    ...
    private AddClaimEntity createImportCertificationEntity(Row row, int index) {
        ...
        final String difference = getCellData(row, 3);


        final Boolean flag = this.checkInvoiceMessage(poCode, claimNo, commodityNo,difference);
        if(flag){
    ...
    private Boolean checkInvoiceMessage(String poCode, String claimNo, String commodityNo, String difference) {
        Boolean flag=true;
