 ...
 ******************************************************************************
 */
public class DeductionInvoiceExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "扣款发票查询导出";
	/**
	 * 文件名后缀
	 */
	...
	public DeductionInvoiceExportThread(IExcelExportService exportLogService,
                                        ExportLogEntity exportLog, PaymentInvoiceQueryService paymentInvoiceQueryService,
	...
	public void run() {
		...
		try {
			...
			try {
				...
				map.put("userCode",exportLog.getUserCode());
				Integer totalRowCount = paymentInvoiceQueryService.queryTotalResult( map).getTotalCount();
				...
				for (int i = 0; i < writeCount; i++) {
					...
					List<PaymentInvoiceUploadEntity> InvoiceEntityList = paymentInvoiceQueryService.queryList( query1);
					List<PaymentInvoiceUploadExcelEntity> list2 = paymentInvoiceQueryService.transformExcle(InvoiceEntityList);
