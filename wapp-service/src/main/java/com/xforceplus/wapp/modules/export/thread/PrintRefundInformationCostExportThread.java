 ...
 ******************************************************************************
 */
public class PrintRefundInformationCostExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "printRefundInformationCostList";
	/**
	 * 文件名后缀
	 */
	 ...
	 * excel的标题
	 */
	private String title = "费用打印退单封面导出";
	...
	public PrintRefundInformationCostExportThread(IExcelExportService exportLogService,
                                                  ExportLogEntity exportLog, CostPrintRefundInformationService costPrintRefundInformationService,
                                                  FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.costPrintRefundInformationService = costPrintRefundInformationService;
	...
	public void run() {
			...
			try {
				...
				try {
					...
					ReportStatisticsEntity reportStatisticsEntity =costPrintRefundInformationService.queryTotalResult(userName,map);
					...
					sheet.setClazz(EnterPackageNumberExcelEntity.class);
					...
					for (int i = 0; i < writeCount; i++) {
						...
						List<EnterPackageNumberExcelEntity> list2=costPrintRefundInformationService.queryListForExcel(userName,query1);
