 ...
 ******************************************************************************
 */
public class InvoiceQuestionExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "InvoiceQuestionCountAndRatioList";
	/**
	 * 文件名后缀
	 */
	 ...
	 * excel的标题
	 */
	private String title = "供应商为题发票数量及比率";
	...
	public InvoiceQuestionExportThread(IExcelExportService exportLogService,
                                       ExportLogEntity exportLog, SupplierIssueInvoiceQuantityandRatioService supplierIssueInvoiceQuantityandRatioService,
                                       FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.supplierIssueInvoiceQuantityandRatioService = supplierIssueInvoiceQuantityandRatioService;
	...
	public void run() {
			...
			try {
				...
				try {
					...
					Integer totalRowCount =supplierIssueInvoiceQuantityandRatioService.problemInvoice(map).size();
					...
					sheet.setClazz(InvoiceQuestionQueryExcelEntity.class);
					...
					for (int i = 0; i < writeCount; i++) {
						...
						List<QuestionInvoiceQuantityAndRatioEntity> list = supplierIssueInvoiceQuantityandRatioService.problemInvoice(query1);
						List<InvoiceQuestionQueryExcelEntity> list2=supplierIssueInvoiceQuantityandRatioService.toExcel(list);
