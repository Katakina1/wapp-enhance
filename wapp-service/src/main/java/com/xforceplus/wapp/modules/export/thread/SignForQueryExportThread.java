 ...
 ******************************************************************************
 */
public class SignForQueryExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "signForQueryList";
	/**
	 * 文件名后缀
	 */
	...
	public SignForQueryExportThread(IExcelExportService exportLogService,
                                    ExportLogEntity exportLog, SignForQueryService signForQueryService,
                                    FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.signForQueryService = signForQueryService;
	...
	public void run() {
			...
			try {
				...
				try {
					...
					Integer totalRowCount =signForQueryService.invoiceMatchCount(map);
					...
					sheet.setClazz(SignForQueryExcelEntity.class);
					...
					for (int i = 0; i < writeCount; i++) {
						...
						List<SignForQueryEntity> InvoiceEntityList = signForQueryService.queryList(query1);
						List<SignForQueryExcelEntity> list2=signForQueryService.transformExcle(InvoiceEntityList);
