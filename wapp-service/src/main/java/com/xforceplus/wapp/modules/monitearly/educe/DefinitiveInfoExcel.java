...
/**
 * 普票跨年度预警导出
 * Created by alfred.zong on 2018/04/19.
 */
public class DefinitiveInfoExcel extends AbstractExportExcel {
    ...
    public DefinitiveInfoExcel(Map<String, List<RecordInvoiceEntity>> map) {
        this.map = map;
    }

    @Override
    protected String getExcelUri() {
        return "export/monitearly/DefinitiveLastYear.xlsx";
    }
    ...
    protected void buildExcel(XSSFWorkbook workBook) {
        ...
        //获取要导出的数据
        final List<RecordInvoiceEntity> list = this.map.get("definitInvoiceEntityList");
        ...
        for (RecordInvoiceEntity recordInvoiceEntity : list) {
            ...
            //签收日期
            setSheetValue(sheet, beginLine, 1, formatDateTime(recordInvoiceEntity.getQsDate()), style);
            //发票代码
    ...
    private String formatDateTime(Date source) {
        return source == null ? "" :new DateTime(source).toString("yyyy-MM-dd");
    }
}
