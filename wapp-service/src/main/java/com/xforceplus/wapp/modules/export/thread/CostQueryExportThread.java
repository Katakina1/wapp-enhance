...
import com.xforceplus.wapp.modules.job.service.impl.AdvanceServiceImpl;
 ...
 ******************************************************************************
 */
public class CostQueryExportThread  extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "costQueryList";
	/**
	 * 文件名后缀
	 */
	...
	public CostQueryExportThread(IExcelExportService exportLogService,
                                 ExportLogEntity exportLog, CostQueryService costQueryService,
                                 FtpUtilService ftpService,BaseUserService baseUserService) {
	...
	public void run() {
			...
			Long userid = Long.valueOf(exportLog.getUserAccount());
			
			String userName =  exportLog.getUserName();

			String venderIds =exportLog.getUserCode();
					//getUserCode();
			//导出条件
			String condition = exportLog.getConditions();

			String costQuery = exportLog.getCostQuery();
			...
			try {
				...
				Map map=prams;
				//获取登陆人的orgtype
				String orgtype = baseUserService.getOrgtype(userid);
				//查询列表数据
				//map.put("venderId","");
				if("8".equals(orgtype)) {
					map.put("venderId",venderIds);
				}
				String orgcode = baseUserService.getOrgtypes(userid);
				System.out.println(costQuery);
				//map.put("loginname","");
				if ("2333".equals(orgcode)){
					map.put("loginname", costQuery);
				}
				// 设置EXCEL名称
				//导出文件名
				...
				try {
					...
					Integer totalRowCount =costQueryService.queryCount(map);
					...
					sheet.setClazz(SettlementExcelEntity.class);
					...
					for (int i = 0; i < writeCount; i++) {
						...
						List<SettlementEntity> InvoiceEntityList = costQueryService.queryList(query1);
						List<SettlementExcelEntity> list2=costQueryService.transformExcle(InvoiceEntityList);
