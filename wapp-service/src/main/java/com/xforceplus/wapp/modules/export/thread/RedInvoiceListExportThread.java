 ...
 ******************************************************************************
 */
public class RedInvoiceListExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "redInvoiceList";
	/**
	 * 文件名后缀
	 */
	...
	public RedInvoiceListExportThread(IExcelExportService exportLogService,
                                      ExportLogEntity exportLog, UploadScarletLetterService uploadScarletLetterService,
									  InvoiceListService invoiceListService,
                                      FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.uploadScarletLetterService = uploadScarletLetterService;
	...
	public void run() {
			...
			try {
				...
				try {
					...
					UploadScarletLetterEntity list1 = uploadScarletLetterService.getTypeById(null,map);
					if(list1.getOrgType().equals("7")){
						Integer totalRowCount = uploadScarletLetterService.queryTotalResultByStore(null,map).getTotalCount();
						...
						Integer totalRowCount = invoiceListService.queryTotalResult(null,map).getTotalCount();
					}

					// 查询总数并 【封装相关变量 这块直接拷贝就行 不要改动】
					Integer totalRowCount =uploadScarletLetterService.queryListAll(map).size();
					...
					sheet.setClazz(RedInvoiceListExcelEntity.class);
					...
					for (int i = 0; i < writeCount; i++) {
						...
						if(list1.getOrgType().equals("7")){
							 list = uploadScarletLetterService.queryListByStore(null,map);
						}else if(list1.getOrgType().equals("1")||list1.getOrgType().equals("5")||list1.getOrgType().equals("2")||list1.getOrgType().equals("4")||list1.getOrgType().equals("8")){
							 list = invoiceListService.queryList(null,map);
						}
						...
						List<RedInvoiceListExcelEntity> list2=uploadScarletLetterService.toExcel2(list);
