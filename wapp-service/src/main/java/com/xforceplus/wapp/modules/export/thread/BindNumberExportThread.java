 ...
 ******************************************************************************
 */
public class BindNumberExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "装订成册导出";
	/**
	 * 文件名后缀
	 */
	...
	public BindNumberExportThread(IExcelExportService exportLogService,
                                  ExportLogEntity exportLog, GenerateBindNumberService generateBindNumberService,
                                  FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.generateBindNumberService = generateBindNumberService;
	...
	public void run() {
		...
		try {
			...
			try {
				...
				Integer totalRowCount = generateBindNumberService.queryTotalResult("", map).getTotalCount();
				...
				sheet.setClazz(BindNumberExcelEntity.class);
				...
				for (int i = 0; i < writeCount; i++) {
					...
					List<GenerateBindNumberEntity> InvoiceEntityList = generateBindNumberService.queryList("", query1);
					List<BindNumberExcelEntity> list2 = generateBindNumberService.transformExcle(InvoiceEntityList);
