 ...
 ******************************************************************************
 */
public class RedInvoiceUploadExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "redInvoiceUpload";
	/**
	 * 文件名后缀
	 */
	...
	public RedInvoiceUploadExportThread(IExcelExportService exportLogService,
                                        ExportLogEntity exportLog, RedInvoiceUploadService redInvoiceUploadService,
                                        FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.redInvoiceUploadService = redInvoiceUploadService;
	...
	public void run() {
		...
		try {
			...
			try {
				...
				ReportStatisticsEntity totalRowCount1 = redInvoiceUploadService.queryTotalResult( map);
				...
				sheet.setClazz(RedInvoiceUploadExcelEntity.class);
				...
				for (int i = 0; i < writeCount; i++) {
					...
					List<RedInvoiceUploadEntity> InvoiceEntityList = redInvoiceUploadService.queryList( query1);
					List<RedInvoiceUploadExcelEntity> list2 = redInvoiceUploadService.transformExcle(InvoiceEntityList);
