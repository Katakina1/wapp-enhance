...
public class InqueryInvoiceExcel extends AbstractExportExcel {
    ...
    protected void buildExcel(XSSFWorkbook workBook) {
        ...
        for (RecordInvoiceEntity recordInvoiceEntity : list) {
            ...
            //发票代码
            setSheetValue(sheet, beginLine, 8, recordInvoiceEntity.getInvoiceCode(), style);
            //发票号码
            setSheetValue(sheet, beginLine, 9, recordInvoiceEntity.getInvoiceNo(), style);
            //发票类型
            setSheetValue(sheet, beginLine, 10, invResult(recordInvoiceEntity.getInvoiceType()), style);
            //开票日期
            try {
            ...
            //签收状态
            setSheetValue(sheet, beginLine, 19, qsResult(recordInvoiceEntity.getQsStatus()), style);
