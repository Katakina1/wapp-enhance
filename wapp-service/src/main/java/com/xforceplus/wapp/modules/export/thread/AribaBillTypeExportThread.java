 ...
 ******************************************************************************
 */
public class AribaBillTypeExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "AribaBillTypeList";
	/**
	 * 文件名后缀
	 */
	 ...
	 * excel的标题
	 */
	private String title = "Ariba业务类型";
	...
	public AribaBillTypeExportThread(IExcelExportService exportLogService,
                                     ExportLogEntity exportLog, AribaBillTypeService aribaBillTypeService,
                                     FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.aribaBillTypeService = aribaBillTypeService;
	...
	public void run() {
			...
			try {
				...
				Map map=prams;
				AribaBillTypeEntity en=new AribaBillTypeEntity();
				Object name=map.get("serviceName");
				Object mcc=map.get("mccCode");
				Object gl=map.get("glAccount");
				if(name!=null){
					en.setServiceName(name.toString());
				}
				if(mcc!=null){
					en.setMccCode(mcc.toString());
				}
				if(gl!=null){
					en.setGlAccount(gl.toString());
				}
				...
				try {
					...
					Integer totalRowCount =aribaBillTypeService.queryTotal("",en);
					...
					sheet.setClazz(AribaBillTypeExcelEntity.class);
					...
					for (int i = 0; i < writeCount; i++) {
						...
						List<AribaBillTypeEntity> list = aribaBillTypeService.queryExcelList(query1);
						List<AribaBillTypeExcelEntity> list2=aribaBillTypeService.toExcel(list);
