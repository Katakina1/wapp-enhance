...
public final class InvoiceCheckHistoryExport extends AbstractExportExcel {
    ...
    protected void buildExcel(XSSFWorkbook workBook) {
        ...
        for (InvoiceCheckModel checkModel : list) {
            ...
            //销方名称
            setSheetValue(sheet, beginLine, 4, assignDefaultValue(checkModel.getXfName()), style);
            // 金额
            setSheetValue(sheet, beginLine, 5, checkModel.getInvoiceAmount() == null ? 0.00 : checkModel.getInvoiceAmount(), style);
            //税额
            setSheetValue(sheet, beginLine, 6, checkModel.getTaxAmount() == null ? 0.00 : checkModel.getTaxAmount(), style);
            //价税合计
            setSheetValue(sheet, beginLine, 7, checkModel.getTotalAmount() == null ? 0.00 : checkModel.getTotalAmount(), style);
            //查验结果
            setSheetValue(sheet, beginLine, 8, assignDefaultValue(checkModel.getCheckMassege()), style);
            beginLine++;
        }
        //采集时间
