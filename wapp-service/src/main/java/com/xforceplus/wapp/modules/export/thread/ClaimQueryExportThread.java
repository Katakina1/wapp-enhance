 ...
 ******************************************************************************
 */
public class ClaimQueryExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "claimQueryList";
	/**
	 * 文件名后缀
	 */
	...
	public ClaimQueryExportThread(IExcelExportService exportLogService,
                                  ExportLogEntity exportLog, ClaimService claimService,
	...
	public void run() {
			...
			try {
				...
				try {
					...
					Integer totalRowCount =claimService.claimQueryList(map).getTotalCount();
					...
					for (int i = 0; i < writeCount; i++) {
						...
						List<ClaimEntity> claimEntityList = claimService.claimQueryList(query1).getResults();
						List<ClaimExcelEntity> list2=claimService.transformExcle(claimEntityList);
