...
import com.xforceplus.wapp.modules.cost.service.CostMatchBUService;
...
@Service
@Transactional
public class CostMatchBUServiceImpl implements CostMatchBUService {

    private final static Logger LOGGER = getLogger(CostMatchBUServiceImpl.class);
    ...
    public List<SettlementMatchEntity> queryList(Map<String, Object> map) {
        List<SettlementMatchEntity> settlement= costMatchBUDao.queryList(map);

        for (SettlementMatchEntity lists : settlement){
            String costDeptIds = costMatchBUDao.costDeptIds(lists.getCostNo());
    ...
    public Integer queryCount(Map<String, Object> map) {
        return costMatchBUDao.queryCount(map);
    }

    @Override
    public List<CostEntity> queryDetail(String costNo) {
        return costMatchBUDao.queryDetail(costNo);
    }

    @Override
    public List<CostEntity> querySelectDetail(String[] costNoArray) {
        return costMatchBUDao.querySelectDetail(costNoArray);
    }

    @Override
    public int selectInvoice(RecordInvoiceEntity entity) {
        return costMatchBUDao.selectInvoice(entity);
    }
        ...
        try {
            ...
            for(List<String> data : list){
                ...
                if(!isMultiple){
                    ...
                    invoiceEntity.setInvoiceType(invoiceTypes(data.get(10)));
    ...
    public String  invoiceTypes(String InvoiceType){
        String value = "";
        if(InvoiceType.equals("普通发票")){
            value = "";
        }else if(InvoiceType.equals("1_增值税专用发票")){
            value = "01";
        }else if(InvoiceType.equals("4_增值税普通发票")){
            value = "04";
        }
        return value;
    ...
    private CostEntity getCostInfo(Long id){
        return costMatchBUDao.getCostInfo(id);
    }
}
