 ...
 ******************************************************************************
 */
public class KnowCenterQueryExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "KnowCenterList";
	/**
	 * 文件名后缀
	 */
	...
	public KnowCenterQueryExportThread(IExcelExportService exportLogService,
                                       ExportLogEntity exportLog, KnowCenterService knowCenterService,
                                       FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.knowCenterService = knowCenterService;
	...
	public void run() {
			...
			try {
				...
				try {
					...
					Integer totalRowCount =knowCenterService.queryCount(map);
					...
					sheet.setClazz(KnowledgeFileExcelEntity.class);
					...
					for (int i = 0; i < writeCount; i++) {
						...
						List<KnowledgeFileEntity> list = knowCenterService.queryList(query1);
						List<KnowledgeFileExcelEntity> list2=knowCenterService.toExcel(list);
