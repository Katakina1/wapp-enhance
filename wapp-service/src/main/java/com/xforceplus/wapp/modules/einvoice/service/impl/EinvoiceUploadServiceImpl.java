...
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.COMMON_ELECTRON_INVOICE_TYPE;
...
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.INVOICE_CODE_END_WITH_ELEVEN;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.INVOICE_CODE_END_WITH_TWELVE;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.INVOICE_CODE_INIT_TYPE;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.INVOICE_CODE_LENGTH;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.INVOICE_CODE_START_WITH;
...
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.INVOICE_KPRQ_FORMAT;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.INVOICE_KPRQ_FORMAT_ZH;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.INVOICE_QS_FAIL_NO_TAX;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.INVOICE_QS_FAIL_ZH;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.INVOICE_QS_FAIL_Z_ZH;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.INVOICE_QS_STATUS_FAIL_ZERO;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.INVOICE_QS_STATUS_SUCCESS_ONE;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.INVOICE_QS_SUCCESS_ZH;
...
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.MAP_KEY_FILE_NAME;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.PDF_FILE;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.RAR_FILE;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.SPECIAL_ELECTRON_INVOICE;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.SUBSTR_REGEX_FOR_FILE;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.TOLL_ELECTRON_INVOICE_TYPE;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.TOLL_INVOICE_QS_FAIL;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.TOLL_INVOICE_QS_FAIL_NO_RECORD;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.UPLOAD_REPEAT_INVOICE;
import static com.xforceplus.wapp.modules.einvoice.constant.Constants.ZIP_FILE_POINT;
...
public class EinvoiceUploadServiceImpl implements EinvoiceUploadService {
    ...
    public List<ElectronInvoiceEntity> uploadElectronInvoice(String schemaLabel, MultipartFile file, UserEntity user) {
        ...
        try {
            ...
            if (fileName.contains(Constants.ZIP_FILE)) {
            ...
            } else if (fileName.contains(RAR_FILE)) {
                ...
                List<File> zipFiles = PdfToImgUtil.findFile(tempPath, ZIP_FILE_POINT);
            ...
            // 获取临时文件夹里的所有pdf文件
            List<File> files = PdfToImgUtil.findFile(tempPath, PDF_FILE);
            //解析并保存电票信息
    ...
    public Boolean deleteElectronInvoice(String schemaLabel, Long id) {
        ...
        //判断此发票是否签收成功
        if (INVOICE_QS_STATUS_SUCCESS_ONE.equals(invoiceEntity.getQsStatus())) {
            //签收成功，删除底账表数据
    ...
    public ElectronInvoiceEntity saveInputElectronInvoice(String schemaLabel, ElectronInvoiceEntity invoiceEntity, UserEntity user) {
        ...
        if (isAlreadySaveInvoice != null) {
            ...
            invoiceEntity.setNotes(UPLOAD_REPEAT_INVOICE);
            ...
            //判断发票类型是电子发票还是通行费发票
            if (COMMON_ELECTRON_INVOICE_TYPE.equals(invoiceType)) {
                //手工录入电子票查验签收并保存
                this.checkInputElectronInvoice(schemaLabel, invoiceEntity, userTaxNos);
            } else if (TOLL_ELECTRON_INVOICE_TYPE.equals(invoiceType)) {
                //签收通行费发票
    ...
    public ElectronInvoiceEntity saveUpdateElectronInvoice(String schemaLabel, ElectronInvoiceEntity invoiceEntity, Long userId) {
        ...
        if (COMMON_ELECTRON_INVOICE_TYPE.equals(this.getInvoiceCodeType(invoiceEntity.getInvoiceCode()))) {
            //查验普通的电子发票
            this.checkUpdateInvoiceAndSave(schemaLabel, invoiceEntity, userTaxNos);
        } else if (TOLL_ELECTRON_INVOICE_TYPE.equals(this.getInvoiceCodeType(invoiceEntity.getInvoiceCode()))) {
            //签收修改的通行费发票
        ...
        if (null != recordInvoice) {
            ...
            if (this.checkUserTaxNoLimit(recordInvoice.getGfTaxNo(), userTaxNos)) {
                ...
                if (recordInvoice.getInvoiceCode().equals(invoice.getInvoiceCode())
                    ...
                    invoice.setResultTip(TOLL_INVOICE_QS_FAIL);
                }
            } else {
                //若没有税号权限
                invoice.setCheckSuccess(Boolean.FALSE);
                invoice.setResultTip(INVOICE_QS_FAIL_NO_TAX);
            }
        } else {
            //如果返回的底账信息为空，则表示查验失败
            invoice.setCheckSuccess(Boolean.FALSE);
            invoice.setResultTip(TOLL_INVOICE_QS_FAIL_NO_RECORD);
        }
    }
    ...
    private void signTollElectronInvoice(String schemaLabel, ElectronInvoiceEntity invoice, List<String> userTaxNos) {
        ...
        RecordInvoice recordInvoice = this.selectRecordInvoice(schemaLabel,
                invoice.getInvoiceCode() + invoice.getInvoiceNo());
        //设置发票类型为通行费发票
        invoice.setInvoiceType(TOLL_ELECTRON_INVOICE_TYPE);
        if (null != recordInvoice) {
            //判读用户是否由此发票的操作权限
            if (this.checkUserTaxNoLimit(recordInvoice.getGfTaxNo(), userTaxNos)) {
                //查询到底账表数据，判读签收
                ...
                if (recordInvoice.getInvoiceCode().equals(invoice.getInvoiceCode())
                    ...
                    invoice.setNotes(INVOICE_QS_FAIL_ZH);
                    invoice.setQsStatus(INVOICE_QS_STATUS_FAIL_ZERO);
                    invoice.setQsType(ELECTRON_INVOICE_QS_TYPE_FIVE);
                }
                //保存信息
                invoice.setId(this.saveElectronInvoice(schemaLabel, invoice));
            } else {
                //若没有税号权限
                invoice.setNotes(INVOICE_QS_FAIL_NO_TAX);
                invoice.setQsStatus(INVOICE_QS_STATUS_FAIL_ZERO);
                invoice.setQsType(ELECTRON_INVOICE_QS_TYPE_FIVE);
            ...
            //若没有查询到底账表数据
            invoice.setNotes(TOLL_INVOICE_QS_FAIL_NO_RECORD);
            invoice.setQsStatus(INVOICE_QS_STATUS_FAIL_ZERO);
            invoice.setQsType(ELECTRON_INVOICE_QS_TYPE_FIVE);
            //保存信息
            invoice.setId(this.saveElectronInvoice(schemaLabel, invoice));
    ...
    private void setTollToSign(String schemaLabel, ElectronInvoiceEntity invoice, RecordInvoice recordInvoice) {
        ...
        invoice.setNotes(INVOICE_QS_SUCCESS_ZH);
        invoice.setQsStatus(INVOICE_QS_STATUS_SUCCESS_ONE);
        invoice.setQsType(ELECTRON_INVOICE_QS_TYPE_FIVE);
        //更新底账表签收日期、签收时间、签收方式
        ...
        if (CHECK_INVOICE_SUCCESS_CODE.equals(recordInvoice.getResultCode())) {
            ...
            if (this.checkUserTaxNoLimit(recordInvoice.getGfTaxNo(), userTaxNos)) {
                ...
                if (CHECK_INVOICE_FAIL_CODE.equals(recordInvoice.getInvoiceStatus())) {
                    //若发票作废在更新保存的信息
                    invoice.setNotes(INVOICE_QS_FAIL_Z_ZH);
                    invoice.setQsStatus(INVOICE_QS_STATUS_FAIL_ZERO);
                    invoice.setQsType(ELECTRON_INVOICE_QS_TYPE_FIVE);
                } else {
                    invoice.setNotes(INVOICE_QS_SUCCESS_ZH);
                    ...
                    invoice.setCreateDate(DateTime.now().toDate());
                    invoice.setUpdateDate(DateTime.now().toDate());
                    invoice.setInvoiceType(COMMON_ELECTRON_INVOICE_TYPE);
                    recordInvoice.setQsStatus(INVOICE_QS_STATUS_SUCCESS_ONE);
    ...
    private List<ElectronInvoiceEntity> analysisElectronFile(String schemaLabel, List<File> files, UserEntity user, String pdfFileName) {
        ...
        try {
            ...
            for (File file : files) {
                ...
                if (map.containsKey(MAP_KEY_FILE_NAME)) {
                    //处理发票解析失败
                    ElectronInvoiceEntity expectionPdf = new ElectronInvoiceEntity();
                    if (pdfFileName.endsWith(PDF_FILE)) {
                        expectionPdf.setPdfName(pdfFileName);
                    } else {
                        expectionPdf.setPdfName((String) map.get(MAP_KEY_FILE_NAME));
                    }
                    expectionPdf.setReadPdfSuccess(false);
                    invoices.add(expectionPdf);
                    continue;
                }
                uploadImg = handler.uploadImg((String) map.get("gfTaxNo"), DateTimeHelper.formatDat(DateTime.now().toDate()), ((File) map.get("zip")).getName());
                //扫描图片唯一识别id
                ...
                //设置图片上传路径
                invoiceEntity.setImgPath(uploadImg + ((File) map.get(ZIP_FILE_POINT)).getName());
                if (pdfFileName.endsWith(PDF_FILE)) {
    ...
    private void checkElectronInvoice(String schemaLabel, List<ElectronInvoiceEntity> invoices, Long userId) {
        ...
        for (ElectronInvoiceEntity invoice : invoices) {
            ...
            if (selectInvoiceEntity != null) {
                invoice.setNotes(UPLOAD_REPEAT_INVOICE);
            ...
            //判断发票类型
            if (COMMON_ELECTRON_INVOICE_TYPE.equals(this.getInvoiceCodeType(invoice.getInvoiceCode()))) {
                //普通电子发票查验
                this.upLoadCheckCallBack(schemaLabel, invoice, userTaxNos);
            } else if (TOLL_ELECTRON_INVOICE_TYPE.equals(this.getInvoiceCodeType(invoice.getInvoiceCode()))) {
                //通行费电子发票查询底账表签收
    ...
    private RecordInvoice getCheckBackData(ElectronInvoiceEntity invoice) {
        ...
        requestData.setInvoiceType(invoice.getInvoiceType() != null ? invoice.getInvoiceType() : COMMON_ELECTRON_INVOICE_TYPE);
    ...
    private Boolean checkUserTaxNoLimit(String taxNo, List<String> userTaxNos) {
        return null != taxNo && !" ".equals(taxNo) && !taxNo.isEmpty() && !DEFAULT_ELECTRON_INVOICE_GF_TAX_NO.equals(taxNo)
    ...
    private String getInvoiceCodeType(String invoiceCode) {
        String invoiceType = INVOICE_CODE_INIT_TYPE;
        //判断是否是特殊的发票
        if (SPECIAL_ELECTRON_INVOICE.contains(invoiceCode)) {
            invoiceType = COMMON_ELECTRON_INVOICE_TYPE;
        }
        //判断是否是普通电子发票或者通行费发票 电子发票和通行费发票长度都是12，且以0开头
        //电子发票已11结尾；通行费发票以12结尾
        if (invoiceCode.length() == INVOICE_CODE_LENGTH && invoiceCode.startsWith(INVOICE_CODE_START_WITH)) {
            if (invoiceCode.endsWith(INVOICE_CODE_END_WITH_ELEVEN)) {
                invoiceType = COMMON_ELECTRON_INVOICE_TYPE;
            }
            if (invoiceCode.endsWith(INVOICE_CODE_END_WITH_TWELVE)) {
                invoiceType = TOLL_ELECTRON_INVOICE_TYPE;
            }
        }
    ...
    private RecordInvoice getRRecordInvoiceFromResponse(ResponseInvoice responseInvoice) {
        ...
        if (responseInvoice != null && CHECK_INVOICE_SUCCESS_CODE.equals(responseInvoice.getResultCode())) {
            ...
            recordInvoice.setQsType(ELECTRON_INVOICE_QS_TYPE_FIVE);
            recordInvoice.setRemark(responseInvoice.getRemark());
