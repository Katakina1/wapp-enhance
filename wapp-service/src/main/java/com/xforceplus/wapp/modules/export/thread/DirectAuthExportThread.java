 ...
 ******************************************************************************
 */
public class DirectAuthExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "DirectAuthList";
	/**
	 * 文件名后缀
	 */
	...
	public DirectAuthExportThread(IExcelExportService exportLogService,
                                  ExportLogEntity exportLog, DirectAuthService scanConfirmService,
                                  FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.scanConfirmService = scanConfirmService;
	...
	public void run() {
			...
			try {
				...
				try {
					...
					Integer totalRowCount =scanConfirmService.queryCount(map);
					...
					sheet.setClazz(DirectAuthQueryExcelEntity.class);
					// 写数据 这个i的最大值直接拷贝就行了 不要改

					List<ComprehensiveInvoiceQueryEntity> claimEntityList = scanConfirmService.queryList(map);
					//for (int i = 0; i < writeCount; i++) {
						List<DirectAuthQueryExcelEntity> list2=scanConfirmService.transformExcle(claimEntityList);
