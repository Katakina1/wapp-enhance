 ...
 * @date 4/19/2018
 */
public class InvoiceCheckImport extends AbstractImportExcel {

    private MultipartFile file;

    public InvoiceCheckImport(MultipartFile file) {
        this.file = file;
    }
    ...
    private ImportCertificationEntity createImportCertificationEntity(Row row, int index) {
        ...
        importCertificationEntity.setInvoiceDate(invoiceDate.replaceAll("-",""));

        //金额
        final String amount = getCellData(row, 4);
        importCertificationEntity.setInvoiceAmount(amount);
        if(StringUtils.isNotEmpty(amount)) {
            final String strAmount = amount.replace(",", "");
            importCertificationEntity.setInvoiceAmount(strAmount);
        }

        //校验码
        final String checkCode = getCellData(row, 5);
        importCertificationEntity.setCheckCode(checkCode);
