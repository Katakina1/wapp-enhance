 ...
 ******************************************************************************
 */
public class PaymentInvoiceUploadExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "paymentInvoiceUpload";
	/**
	 * 文件名后缀
	 */
	...
	public PaymentInvoiceUploadExportThread(IExcelExportService exportLogService,
                                            ExportLogEntity exportLog, PaymentInvoiceUploadService paymentInvoiceUploadService,
                                            FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.paymentInvoiceUploadService = paymentInvoiceUploadService;
	...
	public void run() {
		...
		int serviceType = exportLog.getServiceType();
		...
		try {
			...
			try {
				...
				//map.put("venderid",exportLog.getUserCode());
				ReportStatisticsEntity totalRowCount1 = paymentInvoiceUploadService.queryTotalResult( map);
				...
				sheet.setClazz(PaymentInvoiceUploadExcelEntity.class);
				...
				for (int i = 0; i < writeCount; i++) {
					...
					List<PaymentInvoiceUploadEntity> InvoiceEntityList = paymentInvoiceUploadService.queryList( query1);
					List<PaymentInvoiceUploadExcelEntity> list2 = paymentInvoiceUploadService.transformExcle(InvoiceEntityList);
