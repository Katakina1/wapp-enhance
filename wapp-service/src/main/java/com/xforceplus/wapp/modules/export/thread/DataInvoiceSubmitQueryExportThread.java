 ...
 ******************************************************************************
 */
public class DataInvoiceSubmitQueryExportThread extends BaseThread {
	 ...
	 * 文件名前缀
	 */
	private String fileNamePrefix = "DataInvoiceSubmitQuery";
	/**
	 * 文件名后缀
	 */
	...
	public DataInvoiceSubmitQueryExportThread(IExcelExportService exportLogService,
                                              ExportLogEntity exportLog, DataInvoiceSubmitService dataInvoiceSubmitService,
                                              FtpUtilService ftpService) {
		
		this.exportLogService = exportLogService;
		this.exportLog = exportLog;
		this.dataInvoiceSubmitService = dataInvoiceSubmitService;
	...
	public void run() {
			...
			try {
				...
				try {
					...
					Integer totalRowCount =dataInvoiceSubmitService.queryInvoiceSubmit(map).getTotalCount();
					...
					sheet.setClazz(InvoiceDataExcelEntity.class);
					...
					for (int i = 0; i < writeCount; i++) {
						...
						List<InvoiceDataExcelEntity> list = dataInvoiceSubmitService.queryInvoiceSubmitForExcel(query1);
