...
public abstract class AbstractExportExcel {
     ...
     * @param excelName 文件名
     */
    public void writeXlsm(HttpServletResponse response, String excelName) {
        final Closer closer = Closer.create();
        try {
            //获取文件流
            final InputStream fs = closer.register(Resources.asByteSource(Resources.getResource(getExcelUri())).openStream());
            ...
            //设置响应
            response.setContentType("application/octet-stream");
            response.setHeader("Content-Disposition", "attachment;filename=" + new String(excelName.getBytes("UTF-8"), "ISO8859-1") + ".xlsm");
            //写出
            final OutputStream outputStream = closer.register(response.getOutputStream());
            workBook.write(outputStream);
        } catch (InvalidFormatException e) {
     ...
     * @param excelName 文件名
     */
    public void writeBD(String dirPath, String excelName) {
        final Closer closer = Closer.create();
        try {
            //获取文件流
            final InputStream fs = closer.register(Resources.asByteSource(Resources.getResource(getExcelUri())).openStream());
            ...
            buildExcel(workBook);
            //写出
            final OutputStream outputStream = closer.register(new FileOutputStream(dirPath+excelName));
            workBook.write(outputStream);
        } catch (InvalidFormatException e) {
            LOGGER.info(" invalid format :", e);
        } catch (FileNotFoundException e) {
