...
public class HttpRequestUtils {
	...
	@Value("${proxy.switch}")
	public void setProxySwitch(String proxySwitch) {
		HttpRequestUtils.proxySwitch = proxySwitch;
	}
	 ...
	 */
	public static String httpPost(String param, String url, Integer socketTimeout, HttpHost proxy, CredentialsProvider credsProvider) throws Exception {
		CloseableHttpClient httpclient=null;
		RequestConfig defaultRequestConfig=null;
		//判断开关是否打开
		if("1".equals(proxySwitch)) {
			//把代理设置到请求配置
			defaultRequestConfig = RequestConfig.custom()
					.setProxy(proxy).setSocketTimeout(socketTimeout).setConnectTimeout(600000).build();
		...
		}else if ("0".equals(proxySwitch)){
			httpclient= getHttpClient();
			defaultRequestConfig = RequestConfig.custom().setSocketTimeout(socketTimeout).setConnectTimeout(600000).build();
		}
		String result = null;
		try {
