...
public class ScanMatchServiceImpl implements ScanMatchService {
    private static Logger logger = LoggerFactory.getLogger(ScanMatchServiceImpl.class);
    @Autowired
    ...
    public void posuopeiInvoice() {
        logger.info("--------商品定时任务扫描匹配开始-----------");
        //获取符合匹配信息的数据    部分匹配， 完全匹配，差异匹配
        // 优化分页查询执行
        Integer totalRowCount = scanMatchDao.findMatchCount();
        ...
        for(int i=0;i<writeCount;i++) {
            ...
            List<MatchEntity> list = scanMatchDao.findpoMatchPageDatas(query);
            if (list != null) {
                if (list.size() > 0) {
        ...
        if(list!=null){
            if(list.size()>0){
                for(SettlementEntity entity : list) {
                    ...
                    List<RecordInvoiceEntity> settlementInvoicess = scanMatchDao.findInvoicesByCostNos(costNo);
                    if (settlementInvoices != null) {
                        if (settlementInvoices.size() > 0) {
                            //继续执行
                            if (settlementInvoicess.size() == 0) {
                            ...
                            for (RecordInvoiceEntity vender : scanList) {
                                ...
                                if (!StringUtils.isEmpty(vender.getInvoiceType())) {
                            ...
                            if (settlementInvoices.size() == scanList.size()) {
                                if (venderFailReason.length() == 0) {
                                    ...
                                    if (failReason.length() == 0) {
                                        ...
                                        if (invoices.size() > 0) { //如果有发票数据进行校验
                                            ...
                                            if ("1".equals(status)) {
                                                scanMatchDao.underWay(costNo);
                                                //如果扫描匹配成功，需要在底账表中更新发票的扫描匹配状态
                                                scanMatchDao.updateRecordInvoices(invoices, status, epsNo);
                                        ...
                                        for (RecordInvoiceEntity scanLists : scanList) {
                                ...
                                //数量差异，提示具体少哪些发票数据
                                scanMatchDao.updateCostFailReason(costNo, "2", "发票数量不一致,未扫描成功发票号码：" + failReason.toString());
                                for (RecordInvoiceEntity scanLists : scanList) {
                                    scanMatchDao.updateCostScan(scanLists.getInvoiceCode() + scanLists.getInvoiceNo(), "2", "未扫描成功发票号码：" + failReason.toString());
                                }
                                ...
                                //设置为扫描匹配失败
                                scanMatchDao.updateCostFailReason(costNo, "2", "EPS单中有普通发票或附件，需人工审核");
                                for (RecordInvoiceEntity scanLists : scanList) {
                                    if (!StringUtils.isEmpty(scanLists.getInvoiceCode())) {
                                        scanMatchDao.updateCostScan(scanLists.getInvoiceCode() + scanLists.getInvoiceNo(), "2", "EPS单中有普通发票或附件，需人工审核");
                                    }
    ...
    private boolean checkIsRight(List<RecordInvoiceEntity> list ,RecordInvoiceEntity entity){
        if(list.size()>0){
            ...
            for(RecordInvoiceEntity invoice:list){
                if(!StringUtils.isEmpty(invoice.getInvoiceType()) || !StringUtils.isEmpty(invoice.getInvoiceCode()) || !StringUtils.isEmpty(invoice.getInvoiceNo())) {

                    if (entity.getInvoiceType().equals("04")) {
                        if (invoice.getInvoiceCode().trim().equals(entity.getInvoiceCode().trim()) && invoice.getInvoiceNo().trim().equals(entity.getInvoiceNo().trim()) && invoice.getInvoiceType().trim().equals(entity.getInvoiceType().trim()) &&
                                invoice.getTotalAmount().toString().trim().equals(entity.getTotalAmount().toString().trim())
                                && invoice.getInvoiceDate().trim().equals(entity.getInvoiceDate().trim())) {
                            return true;
                        }
                    }else{
                        if (invoice.getInvoiceCode().trim().equals(entity.getInvoiceCode().trim()) && invoice.getInvoiceNo().trim().equals(entity.getInvoiceNo().trim()) && invoice.getInvoiceType().trim().equals(entity.getInvoiceType().trim()) &&
                                invoice.getTaxAmount().toString().trim().equals(entity.getTaxAmount().toString().trim()) && invoice.getInvoiceAmount().toString().trim().equals(entity.getInvoiceAmount().toString().trim())
                                && invoice.getInvoiceDate().trim().equals(entity.getInvoiceDate().trim())) {
                            return true;
                        }
                    }
