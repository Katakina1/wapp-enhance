 ...
 **/
public class InqueryInvoiceExcel extends AbstractExportExcel {

    private Map<String, List<RecordInvoiceEntity>> map;

    public InqueryInvoiceExcel(Map<String, List<RecordInvoiceEntity>> map) {
        this.map = map;
    }
    ...
    protected String getExcelUri() {
        return "export/signin/RecordInvoiceInquery.xlsx";
    }
    ...
    protected void buildExcel(XSSFWorkbook workBook) {
        ...
        for (RecordInvoiceEntity recordInvoiceEntity : list) {
            //序号
            setSheetValue(sheet, beginLine, 0, String.valueOf(beginLine));
            //签收结果
            ...
            //购方名称
            setSheetValue(sheet, beginLine, 7, recordInvoiceEntity.getGfName(), style);
            //销方名称
            setSheetValue(sheet, beginLine, 8,recordInvoiceEntity.getXfName(), style);
            //金额
            setSheetValue(sheet, beginLine, 9, formatBigDecimal(recordInvoiceEntity.getInvoiceAmount()), style);
            //税额
            setSheetValue(sheet, beginLine, 10, formatBigDecimal(recordInvoiceEntity.getTaxAmount()), style);
