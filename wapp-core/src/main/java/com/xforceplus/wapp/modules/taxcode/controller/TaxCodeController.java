...
public class TaxCodeController {
    ...
    public R<PageResult<TaxCode>> getOverdue(@ApiParam("页数") @RequestParam(required = false, defaultValue = "1") Long current,
        ...
        val page = taxCodeService.page(current, size, goodsTaxNo, itemName, itemNo);
    ...
    public R<Boolean> delOverdue(@RequestBody @Validated IdsDto idsDto) {
        long start = System.currentTimeMillis();
        List<TaxCodeEntity> entities = idsDto.getIds().stream().distinct().map(it -> TaxCodeEntity.builder()
                .id(it).deleteFlag(String.valueOf(System.currentTimeMillis())).build()).collect(Collectors.toList());
