...
//				 throw new EnhanceRuntimeException("调用集成平台错误:" + e.getMessage());
    ...
    private Boolean verifyInvoice(DiscernResult discernResult) {
        ...
        if (null != detailEntity) {
            ...
            if (!verificationResponse.isOK()) {
                ...
                int num = electronicUploadRecordDao.countNum(detailEntity.getBatchNo(), "0");
                this.electronicUploadRecordDao.increaseFailureNum(detailEntity.getBatchNo(), num);
                return true;
            }
            ...
            if (null != entity) {
                if ("qs".equalsIgnoreCase(discernResultDetail.getInvoiceType()) || "qc".equalsIgnoreCase(discernResultDetail.getInvoiceType())) {
                    ...
                    resultEntity.setReason("全电发票必须上传xml文件，请重新上传");
                    resultEntity.setOfdStatus(Constants.SIGIN_NONE_BUSINESS_FAIL);
                ...
                if (verificationResponse.isOK()) {
                    ...
                    resultEntity.setXfVerifyTaskId(verificationResponse.getResult());
                    noneBusinessService.updateById(resultEntity);
                    return true;
                } else {
                    log.warn("发票代码:{},发票号码：{}，发票验真请求失败:{}", invoice.getInvoiceCode(), invoice.getInvoiceNo(),
                    ...
                    resultEntity.setId(entity.getId());
                    resultEntity.setVerifyStatus(Constants.VERIFY_NONE_BUSINESS_FAIL);
                    resultEntity.setReason(verificationResponse.getMessage());
                    resultEntity.setOfdStatus(Constants.SIGIN_NONE_BUSINESS_FAIL);
                    noneBusinessService.updateById(resultEntity);
                }
    ...
    public String discern(byte[] pdfPathList) {
        ...
        // 0-未定义；1-裸扫发票；2-A4纸发票；3-拍照；4-上传文件
        discernRequest.setScene("4");
        discernRequest.setFile(gzip(pdfPathList));
        discernRequest.setFileSuffix(".pdf");
        //属地系统需要增加callbackUrl
        //https://wiki.xforceplus.com/pages/viewpage.action?pageId=30018277
        discernRequest.setCallbackUrl(callbackUrl);
